...
3. To complete the rule statement, under **When a new file is added**:
    ...
    3. Select **choose a library**, and then select the document library you want the file moved or copied to.
       ...
       If you try to set up a rule to move or copy a file to a library that already has a move  or copy rule applied, you'll receive a message saying that you need to disable all move or copy rules on the destination library. To disable a rule, see [Manage a rule](#manage-a-rule).
...
When a file is moved or copied, you'll see an update in the source library activity feed.

In the document library, in the upper-right corner of the page, select the **Details** pane to view the recent history, activity, and rules applied to the library.
