...
1. Select the **Type of test**. There are two test types supported:
   ...
   - A **Flown Driven test** test allows you to arrange your test scripts with enhanced flow control. To help you comprehensively validate the impact of an in-place Windows upgrade, you can use flow driven tests to execute your tests on both the baseline OS and target OS with a side-by-side test result comparison.

Note: User can even select Pre-intalled Microsoft apps option. Enabling this option will install the Microsft app, like office, before the user application is installed. 
