---
title: "Create DNS records using Windows-based DNS"
f1.keywords:
- NOCSH
ms.author: pebaum
author: pebaum
...
description: "Learn to verify your domain and set up DNS records for email, Skype for Business Online, and other services at Windows-based DNS for Microsoft 365."
---

# Create DNS records using Windows-based DNS
...
To get started, you need to [find your DNS records in Windows-based DNS](#find-your-dns-records-in-windows-based-dns) so you can update them. Also, if you're planning to synchronize your on-premises Active Directory with Microsoft 365, see [Non-routable email address used as a UPN in your on-prem Active Directory](#non-routable-email-address-used-as-a-upn-in-your-on-prem-active-directory).
...
- From the MX row in the Exchange Online section of the Add DNS records page in Microsoft 365, copy the value listed under Points to address. You'll use this value in the record you're creating in this task. 
...
- In the **New Resource Record** dialog box, make sure that the fields are set to precisely the following values: 
    - Host Name: 
    - @Address: Paste the Points to address  value that you just copied from Microsoft 365 here.  
    - Pref: 
- Select **Save Changes**.
...
Add the CNAME records that are required for Microsoft 365. If additional CNAME records are listed in Microsoft 365, add those following the same general steps shown here.
...
Before you add the DNS records to set up your Microsoft 365 services, Microsoft 365 has to confirm that you own the domain you're adding. To do this, you add a record, following the steps below.
  
> [!NOTE]
> This record is used only to verify that you own your domain; it doesn't affect anything else. 
  

1. Gather information from Microsoft 365.  <br/> 
...
4. On the **Add a domain to Microsoft 365** page, select **Start step 1**. 
...
- Host Name: @
- Type: TXT
- Address: Paste the Destination or Points to Address value that you just copied from Microsoft 365 here.  
- Select **OK** > **Done**.
...
- Go back to Microsoft 365 and follow the steps below to request a verification check. The check looks for the TXT record you added in the previous step. When it finds the correct TXT record, the domain is verified.  
