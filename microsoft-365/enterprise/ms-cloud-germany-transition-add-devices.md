...
Ensure all of your devices are registered with the worldwide endpoint before your organization is disconnected from the Microsoft Cloud Deutschland cloud.  
...
To check that your devices are registered in public cloud, you should export/download the devices from Azure AD portal to an excel file, and then filter the devices that are registered (filter using registeredTime column) after migration stage Separate from Microsoft Cloud Deutschland.
...
To determine whether the Windows 10 device is Azure AD registered, you can run the following command on the device:
...
How to un-register and re-register the device on Android with the Authenticator app:
...
How to un-register and re-register an Android device with the Android Settings page
...
2. Tap the **Settings** icon in the top right corner. If you don’t see the **Settings** icon, you might be not using the latest Authenticator version.
