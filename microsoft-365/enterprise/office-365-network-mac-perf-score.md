...
## Tenant network assessments, office location and remote worker network assessments

A network assessment measures the design of the network perimeter of a location to Microsoft's network. Improvements to the network perimeter of office locations is best done at each office location.

We show a network assessment value for the whole Microsoft 365 tenant on the network performance overview page which is a weighted average of the network assessments for all office locations. There is also a specific network assessment value for each detected office location on that location's summary page as well as the network assessment value for remote workers in that city. We do not identify individual users working from home.
