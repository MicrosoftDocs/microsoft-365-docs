...
Non-enterprise organizations can also deploy Microsoft 365 Enterprise and realize the business value of an integrated and secure infrastructure that enables teamwork and unlocks creativity. The non-enterprise typically has a:

- Small amount of on-premises IT infrastructure, such as email and file servers and an Active Directory Domain Services (AD DS) domain, or none at all.
- Small IT staff, most of whom are IT generalists, rather than specialists in a specific technology or workload such as networking or email.

For your smaller, non-enterprise organization, Microsoft offers Microsoft 365 Business. However, there are reasons why you might need Microsoft 365 Enterprise, such as:
...
- Your organization needs the advanced productivity, security, and analytics that are not available with Microsoft 365 Business.

This article steps you through the deployment of the foundation infrastructure of Microsoft 365 Enterprise. 
...
Next, you need to configure additional security for Microsoft 365. Follow the instructions in [Configure increased security](https://docs.microsoft.com/office365/securitycompliance/tenant-wide-setup-for-increased-security#tune-threat-management-policies-in-the-office-365-security--compliance-center).

## Phase 1: Networking

Non-enterprise organizations typically have local Internet connections in each office and do not use proxy servers, firewalls, or packet inspection devices. The Internet service provider (ISP) at each office has a regionally local DNS server so that traffic is directed to the Microsoft 365 cloud servers that are geographically closest to your offices and their on-premises users.
...
Here is a visual summary with the Phase 1 element highlighted. **Your organization** can be multiple offices, each of which has a local Internet connection with an ISP that uses a regionally local DNS server. Through the ISP, each office can reach the nearest Microsoft network location for the resources of your Microsoft 365 subscription.
...
If your organization requires high security and you have the Microsoft 365 Enterprise E5 subscription, use Azure AD Privileged Identity Management to enable just-in-time administrator access. See [Set up on-demand global administrators](identity-designate-protect-admin-accounts.md#set-up-on-demand-global-administrators) for more information.

### Recommendations for groups

If you have an on-premises AD DS domain, continue to use those groups in Microsoft 365 as groups in Azure AD.
...
If you have on-premises AD DS, configure Azure AD Connect on a server with password hash synchronization (PHS) to the Azure AD tenant of your Microsoft 365 Enterprise subscription. See [Synchronize identities](identity-azure-ad-connect.md) for more information.

### More secure user access with conditional access policies

Azure AD evaluates the conditions of a user sign-in and uses conditional access policies to grant or deny access and impose further actions that must be taken to complete the sign-in. For example, if Azure AD determines that the sign-in is happening under medium or high-risk conditions, it can require the user to perform multi-factor authentication to complete the sign-in.
...
| ![](./media/deploy-foundation-infrastructure-non-enterprises/identity-aad-settings.png) | The set of Azure AD settings for authentication, securing global accounts, and making it easier to manage accounts and licenses. |
| ![](./media/deploy-foundation-infrastructure-non-enterprises/identity-aad-caps.png) | The set of Azure AD conditional access policies. |
|||

## Phase 3: Windows 10 Enterprise

To ensure that your Windows 10 Enterprise devices are integrated into the identity and security infrastructure of your organization, here are your options:
...
- Hybrid (you have on-premises AD DS)
  ...
  For each Windows 10 Enterprise device already joined to your AD DS, join them to the Azure AD tenant. See [How to configure hybrid Azure Active Directory joined devices](https://go.microsoft.com/fwlink/p/?linkid=872870) for the instructions.

  For each new Windows 10 Enterprise device, join them to your AD DS, and then join them to the Azure AD tenant.

  You must also enroll your Windows 10 Enterprise devices for mobile device management. See [Enroll a Windows 10 device with Intune by using a Group Policy](https://go.microsoft.com/fwlink/p/?linkid=872871) for the instructions.

Once installed and joined, each Windows 10 Enterprise device will automatically install updates from the Windows Update for Business cloud service. There is no need in a non-enterprise organization to set up an infrastructure to distribute and install updates.
...
Microsoft 365 Enterprise includes Office 365 ProPlus, the subscription version of Microsoft Office. Like Office 2016 or Office 2019, Office 365 ProPlus is installed directly on your client devices. Office 365 ProPlus updates itself with new features on a regular basis. See [About Office 365 ProPlus in the enterprise](https://docs.microsoft.com/deployoffice/about-office-365-proplus-in-the-enterprise) for more information.
...
In either case, the administrator or the user signs in with the user’s account name and password to the Office 365 portal at https://portal.office.com. On the Microsoft Office Home tab, click Install Office and step through the installation process.

Feature updates to the Office 365 ProPlus client applications are downloaded monthly by each computer on which it is installed. There is no need in a non-enterprise organization to set up an infrastructure to distribute and install updates. 
...
Microsoft 365 Enterprise includes Microsoft Intune for mobile device management. With Intune, you can manage iOS, Android, macOS, and Windows devices to protect access to your organization's resources, including your data. Intune integrates with the users, groups, and computer accounts of Azure AD.

Intune provides two types of mobile device management:

- Mobile device management (MDM) is when devices get enrolled in Intune. Once enrolled, they are managed devices and can receive any policies, rules, and settings used by your organization. These types of devices are typically owned by your organization and issued to your employees.
...
Not all the documents and email in your organization have the same value. For example, normal correspondence between employees and the documents that they work on have a certain baseline value. Financial records, customer data, and your intellectual property have a higher value. Microsoft 365 Enterprise has a host of information protection features that allow you treat these classifications of data differently by applying different levels of governance, security, and protection.
...
- Sensitive
  ...
  Examples include financial and legal information and research and development data for new products.
...
- Custom sensitive information types
  ...
  Microsoft 365 supplies a wide selection of sensitive information types, such as health service and credit card numbers. If you do not find one in the Microsoft 365-supplied list that you need, you can create your own.
...
- Sensitivity labels
  ...
  You can label email or documents with a named sensitivity label so that the additional levels of security can be applied. Examples are as watermarks, encryption, and permissions that specify who is allowed to access the email or document and what they are allowed to do.
...
For example, you need to create a RESEARCH sensitivity label to protect the email and documents of your research team. You determine that some researchers must have the ability to change documents marked with the RESEARCH sensitivity label and other non-research employees need have the ability to only view documents marked with the RESEARCH sensitivity label. This means you need to create and manage two additional groups:

- RESEARCH-ALL
- RESEARCH-VIEW

These groups are part of the configuration of the RESEARCH sensitivity label’s permissions.

You must keep these groups in mind when you change staff or hire new employees for departments that use sensitivity labels configured with group-based permissions.
...
Once all of this infrastructure is in place, you can onboard an employee for Microsoft 365 Enterprise with these processes.

### A new Windows 10 Enterprise device

Before giving an employee a new Windows 10 Enterprise device, you should do the following:
...
- For hybrid identity
  ...
  Join the device to your AD DS, join the device to your Azure AD tenant, and then enroll the device in Intune.
...
### New employee with cloud-only user account

As part of the initial onboarding for your organization for new employees when your organization is using cloud-only identity, add the new user account to these groups:
...
For new employees or existing employees with an AD DS user account, instruct them to:

1. Sign into their devices with their user account credentials.
2. Using a browser, have them sign in to the Office 365 portal at https://portal.office.com.
3. From the Office 365 home page, click **Install Office** to install Office 365 ProPlus on their device.
...
### Results for your infrastructure

After the build-out and configuration of the Microsoft 365 Enterprise infrastructure, you should have:
...
Here is a visual summary of the infrastructure if your organization uses hybrid identity, which includes your AD DS domain, an Azure AD Connect server, and synchronized users and groups.
...
Deploy your [workloads and scenarios](deploy-workloads.md) take advantage of all the features and configuration of your foundation infrastructure.
