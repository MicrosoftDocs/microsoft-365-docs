...
This guidance is designed for administrators of _Tenants_ preparing their Microsoft 365 _Tenant_ to be expanded to additional Geographies in accordance with the customer's need to meet data residency requirements.

In a Multi-Geo configuration, the customer's Microsoft 365 _Tenant_ consists of a _Primary Provisioned Geography_ location and multiple _Satellite Geography_ locations. This is a single _Tenant_ that spans across multiple _Geography_ locations retaining the simplicity of single-tenant administration and full-fidelity collaboration experiences across the _Geographies_ as necessary. 
...
2. Configure any workloads which require customer specific settings for Multi-Geo.

3. Set your users' Preferred Data Location (PDL) to the desired _Satellite Geography_ location. When a new user’s OneDrive for Business site, or Exchange Online mailbox, or Teams chat store is provisioned, it is provisioned to their PDL if the PDL value is configured prior to assigning them a Microsoft 365 license. When an existing user's PDL value is set to a new value, then their existing Exchange Online mailbox and Teams chat store will automatically be migrated to the new geography.

4. Migrate your users' existing OneDrive for Business sites from the _Primary Provisioned Geography_ location to their _Satellite Geography_ data location as needed. OneDrive for Business sites do not migrate automatically like Exchange Online mailboxes or Teams chat stores.

See [Configure Microsoft 365 Multi-Geo](multi-geo-tenant-configuration.md) for details on each of these steps.

Please see the [Availability section](microsoft-365-multi-geo.md#microsoft-365-multi-geo-availability) of the Microsoft 365 Multi-Geo Overview page for the _Geographies_ that can be a _Satellite Geography_ location where you can host data for OneDrive for Business, SharePoint Online, Exchange Online, and Microsoft Teams. 
...
Each user should have a _preferred data location_ (PDL) set so that Microsoft 365 can determine in which _Geography_ location to provision or relocate their data to. The user's preferred data location must match one of the available _Geographies_. While the PDL field is not mandatory, we do recommend that a PDL value be set for all users. Users without a PDL value set will be provisioned in the _Primary Provisioned Geography_. If the PDL value is not a valid value, this will also result in the user's data being provisioned in the _Primary Provisioned Geography_.
