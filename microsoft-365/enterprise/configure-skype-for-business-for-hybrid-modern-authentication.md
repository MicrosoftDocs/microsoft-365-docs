...
After you double-check that you meet the [prerequisites](hybrid-modern-auth-overview.md) to use Modern Authentication (see the note above), you should create a file to hold the info you'll need for configuring HMA in the steps ahead. Examples used in this article:
...
You need internal and external web service URLs for all SfB 2015 pools deployed. To obtain these URLs, run the following command from Skype for Business Management Shell:
...
If you're using a Standard Edition server, the internal URL is blank. In this case, use the pool fqdn for the internal URL.
...
3. If the internal **or** external SFB URLs from on-premises are missing (for example, https://lyncwebint01.contoso.com and https://lyncwebext01.contoso.com) you need to add those specific records to this list.
