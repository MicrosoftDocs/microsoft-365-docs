...
As described in the migration [preword](ms-cloud-germany-transition-add-pre-work.md#Exchange-Online-Hybrid-configuration), **before the migration step phase 5 begins,** any Exchange Online hybrid customer needs to run the latest version of the HCW in Office 365 Germany mode to prepare on-premises configuration for the migration to Office 365 global.<br>
On **completion of the migration phase 5** (when the Message Center notice is published), you need to run the HCW against using Office 365 Worldwide settings to point your on-premises systems to the Office 365 Global service. Additional DNS updates may be required if you use custom domains.
