...
| Term | Definition |
|:-----|:-----|
|Advanced Data Residency (ADR) |A Microsoft 365 add-on service that guarantees customer data residency for a defined set of services, extending from data-at-rest (Tier 1) to include data processing commitments (Tier 2/Tier 3) for eligible services and geographies. See [Table 2](m365-dr-overview.md#table-2-durable-commitments-on-data-location-by-microsoft-365-service) below and [Advanced Data Residency](m365-dr-commitments.md) for details. |
...
|Committed Geography |_Committed Geography_ refers to the location where Microsoft will store in scope customer data at rest for certain Microsoft 365 services. This location is determined by the _Durable Commitments on Data Location_ applicable to the _Tenant_. Visit [Available Data Residency by Country/Region](m365-dr-overview.md#table-3-durable-commitments-on-data-location-by-geography) for more information. |
...
|Local Region Data Residency (LRDR) |A data residency framework where customer data is confined to a single _Local Region Geography_. LRDR provides two types of commitments:<br />• **Data at Rest (Tier 1)**: Customer content stored within one _Local Region Geography_<br />• **Data Processing (Tier 2/Tier 3)**: Computations, transformations, and AI inference performed within the same _Local Region Geography_<br /><br />LRDR is incompatible with Multi-Geo Capabilities, which by design distribute data across multiple _Local Region Geographies_. |
|LRDR Processing Eligible Geography |Geographies where Tier 2/Tier 3 processing commitments are available for Microsoft 365 Copilot. Includes: Australia, Canada, Germany, India, Italy, Japan, Malaysia, Poland, Singapore, South Africa, Spain, Sweden, Switzerland, United Arab Emirates, United Kingdom, and United States. Availability is phased; see the [Data Residency for Microsoft 365 Copilot and Copilot Chat](m365-dr-workload-copilot.md) page for details. |
...
|Normal Operations |Standard operational state where Microsoft 365 services are fully available and functional, with no active service incidents, scheduled maintenance, or outages. Processing commitments apply during Normal Operations; certain exclusions may apply during non-normal operations. |
|Tier 1 Commitment |_Local Region Data Residency_ (LRDR) for data at rest. Customer data is stored within the relevant _Local Region Geography_ for eligible customers, according to Microsoft Product Terms and Advanced Data Residency. |
|Tier 2 Commitment |_Local Region Data Residency_ (LRDR) for data processing. Microsoft 365 Copilot interactions (user prompts and responses from Azure OpenAI) are processed in the customer's _Local Region Geography_ under _Normal Operations_. Extends Tier 1 storage commitments with processing guarantees. Available for eligible tenants with Advanced Data Residency. |
|Tier 3 Commitment |_Local Region Data Residency_ (LRDR) for data processing with in-region redundancy. Microsoft 365 Copilot interactions are processed in the customer's _Local Region Geography_ with in-region redundancy. Future capability extending Tier 2 commitments. |
...
1. _Multi-Geo_ subscription: allows customers to assign data location for Exchange Online, SharePoint, OneDrive, Microsoft Teams, and Microsoft 365 Copilot and Copilot Chat to any supported _Geography_. For more information, see [Multi Geo Data Residency](microsoft-365-multi-geo.md).
...
| Geography  | Exchange Online | SharePoint, OneDrive | Teams | Microsoft 365 Copilot and Copilot Chat | MDO P1 | Microsoft 365 web apps | Viva Connections | Purview |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
