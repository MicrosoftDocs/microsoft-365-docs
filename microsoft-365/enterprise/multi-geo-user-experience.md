...
- Microsoft 365 Profile (/graph/api/resources/profile?view=graph-rest-beta), which constitutes the information that can be viewed by other users within the tenant about the Microsoft 365 person. This includes the user's Microsoft Entra ID details along with other information like position and photo (https://aka.ms/profileimage). The fields and attributes that are exposed can be controlled by the tenant admin (/graph/add-properties-profilecard).

- My Microsoft 365 People dataset which represents the list of the user's Microsoft 365 contacts (/graph/api/resources/contact?view=graph-rest-1.0) along with the user’s version of their contacts' Microsoft 365 profile. This edited or unedited version of another user’s profile will always remain private in the current user’s mailbox and won't be replicated anywhere.
