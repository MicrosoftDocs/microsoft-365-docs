...
To add a large number of accounts to SharePoint sites and give them permissions, you can use the Microsoft 365 admin center, individual PowerShell commands, or PowerShell CSV file. Of these choices, the CSV file is the fastest way to automate this task.
...
For example, let's create a CSV file to define a group of site collections, groups, and permissions. Next, we'll create a CSV file to populate the groups with users. Finally, we'll create and run a simple Windows PowerShell script that creates and populates the groups.
...
The script imports the CSV file contents and uses the values in the columns to populate the parameters of the **New-SPOSiteGroup** and **Add-SPOUser** commands. In our example, we're saving this to theO365Admin folder on drive C, but you can save it wherever you want.
