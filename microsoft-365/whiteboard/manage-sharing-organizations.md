...
During a one on one or group call, you may start sharing a Whiteboard. Similar limitations apply to who can share a Whiteboard regarding scenarios where users from different organizations are involved in a call. When all members of the call are from the same organization, any person can start and access the Whiteboard. For calls involving users from different organizations, only some users will be able to access the Whiteboard sharing button. During a one on one call, only the original user to create a conversation/call (the first person to send a message or call another user, whichever occurs first) between the two users can access the Whiteboard sharing button from the drop down share tray button. This can not be changed after two users have started a conversation with each other. Deleting the chat involving the two users will not "start over" the chat, therefore, this will not reset who created/started the chat. Having the other user call will also not change who can share the whiteboard, even if a Whiteboard has not been shared yet. The purpose of this limitation is to avoid providing an out-of-organization user access to a Whiteboard unless sharing starts from an in-organization user.
