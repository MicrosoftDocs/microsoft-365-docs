...
|Not compliant|The value detected in the existing configuration deployed to the customer tenant is not equivalent to the value in the deployment plan for one or more targeted users.|
|Missing|There is no value detected in the tenant for a setting that is included in the deployment plan.|
|Extra|There is a value detected in the tenant for a setting that is not included in the deployment plan.|
...
|Excluded|The user has been Excluded from the subtask.<p>**NOTE**: When a user is Excluded from a subtask, status detection and reporting will be updated accordingly, but existing configurations will not be affected.</p>|
|Not licensed|The user is not licensed for the services required to deploy the prescribed configuration.<p>**NOTE:** Does not apply to users with Not targeted status.</p>|
|Not targeted|The user is not targeted for the subtask. For example, a user that is not an admin is reported as Not targeted for a subtask that is assigned only to admins.|
