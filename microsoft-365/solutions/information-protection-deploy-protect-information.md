...
A number of information protection controls can be employed in your environment to help address data privacy compliance needs, including (per Compliance Score) a number that are specific to GDPR, HIPAA-HITECH, CCPA, and LGPD. 

These controls primarily fall into the following solution areas, which are explored further below:

- Sensitivity labels (unified)
- Data Loss Prevention (Office 365)
- Office message encryption (OME)
- Teams and sites access controls (integrated scenario)
- 
...
Your organization may also wish to protect Office 365 content for other purposes (i.e., other compliance needs or for business reasons), so establishing your information protection scheme for data privacy should be done as part of overall information protection planning, implementation and management.

To help you get started with an information protection scheme in Microsoft 365, inclusive of that applicable to data privacy regulations, a short list of related capabilities and improvement actions for Microsoft 365 are listed in the following section. This purposefully omits capabilities that are increasingly considered legacy relative to newer methods (e.g. SharePoint and OneDrive IRM).
...
Microsoft information protection solutions include a number of integrated capabilities across Office 365, Azure and Windows. In Office 365, principal information protection solutions include:

- Sensitive information types (covered previously in article 2. Assess data privacy risks and identify sensitive information - Sensitive information types).
- Sensitivity labels
...
In addition, site and library level protection are important mechanisms to include in any protection scheme, so these topics are discussed in this section as well.

These are shown in this high-level diagram and explored further below (capabilities in the blue lettering are highlighted in this article).

FIGURE
 
For additional information on other MIP capabilities outside of Office 365, the reader is referred to articles on Microsoft Cloud Application Security (MCAS), Azure Information Protection (AIP), Intune/Endpoint Manager, and Windows Information Protection.

## Sensitivity Labels (unified) 

General reference: Sensitivity labels 
...
1. Understand the modern sensitivity label capability in Microsoft 365 and business reasons for their application
   - Business requirements. Establish the business reasons for applying sensitivity labels in your enterprise (e.g. data privacy requirements for information protection).
...
2. Rationalize modern sensitivity labelling with past/alternative methods 

- Azure Information Protection. A new modern sensitivity labelling scheme may need to be reconciled with any existing Azure Information Protection (AIP) labelling implementation.
- Office Message Encryption (OME). If you are planning to use modern sensitivity labelling for email protection and existing email encryption methods like Office Message Encryption are in place, they can co-exist, but you should understand the scenarios in which either should be applied. Refer to the section Office message encryption (OME), which includes a table comparing modern sensitivity label-type protection with OME-based protection.

3. Plan for integration into a broader information protection scheme. On top of coexistence with OME, modern sensitivity labels can be used along-side capabilities like Office 365 data loss prevention (DLP) (covered later in this section) and Microsoft Cloud App Security (refer to Sensitivity Labels and Microsoft Cloud App Security) to achieve your data privacy-related information protection goals. 

4. Develop a sensitivity label classification and control scheme. A good article to help you with developing a sensitivity labelling classification scheme can be found on the Services Trust Portal: Data Classification and Sensitivity Label Taxonomy.
...
#### Example sensitivity labelling-scheme

An example sensitivity-labelling scheme is provided here for reference. Yours may vary, but consider this a good starting point (note that the sensitivity labels for sites type have a “_site” suffix appended to them to differentiate them from content-targeted labels):
...
Content (client-side and service-side application)	Non-business	N/A	Data does not belong to <your org>, is not encrypted and cannot be tracked or revoked.

Not to be used for personal data.
	...
	Highly confidential	Recipients only	Data is classified and protected with “Do Not Forward” rules. Recipients can read but cannot forward, print, or copy content.

Conversation owner has full permission to their message and all replies.
...
### Create and deploy sensitivity labels for sites, Groups and Teams (container level)

Reference: Sensitivity labels for sites, groups and Teams (container-level)

When you create sensitivity labels in the Microsoft 365 compliance center, you can now apply them to the following containers: Microsoft Teams sites, Microsoft 365 groups (formerly Office 365 groups), and SharePoint sites. Use the following label settings to help protect the content in those containers:
...
End-user (person provisioning a site, group, or Team) experience:
- Team
 
- Site
 
When you apply this label to a supported container, the label automatically applies the configured options to the connected Team or site.
Note that content in those containers, however, do not inherit the labels for settings such as the label name, visual markings, or encryption. 
You can also view how sites are labelled in the SharePoint admin center:
 
So that users can label their documents in SharePoint sites or team sites, enable sensitivity labels for Office files in SharePoint and OneDrive (this is covered further below).

Example application in a data privacy context: for containers that will be used for storing content with sensitive personal data, prevent external sharing, mark private and require managed devices. 
...
Sensitivity labels for content allow you to encrypt content, watermark content, and define other controls on content worked with in Office applications, including Outlook and Office on the web.
 
When you're ready to start protecting your organization's data by using sensitivity labels:

1. **Create the labels.** Create and name your sensitivity labels according to your organization's classification taxonomy for different sensitivity levels of content. For more information on developing a classification taxonomy, refer to the previously referenced article: Data Classification and Sensitivity Label Taxonomy.
...
- When you have published sensitivity labels from the Microsoft 365 compliance center, they start to appear in Office apps for users to classify and protect data as it's created or edited.

Example application in a data privacy context: 

- Manually apply sensitivity label with encryption and other rules to email or content containing sensitive personal information.

>[!Note]
>Sensitivity labels applied to email (with encryption) have some overlapping functionality with OME, so refer to the section below: “Secure email scenarios comparison.”
>

### Client-side auto-labeling when users edit documents or compose (also reply or forward) emails

Reference: Apply a sensitivity label to content automatically

When you create a sensitivity label, you can automatically assign that label to content (including email) when it matches conditions that you specify.
...
For configuration instructions, see How to configure auto-labeling for Office apps.

Example application in a data privacy context: 

- Auto-apply sensitivity labels for content containing sensitive personal information.

### Service-side auto-labeling when content is already saved (in SharePoint Online or OneDrive for Business) or emailed (processed by Exchange Online)
...
For configuration instructions, see How to configure auto-labeling policies for SharePoint, OneDrive, and Exchange.

Example site library with sensitivity labels auto-applied:
 
FIGURE

Example application in a data privacy context: 

- For sites of concern, push sensitivity labels for automatic encryption of content containing sensitive personal information.

## Data Loss Prevention (Office 365)

Reference: Data loss prevention

Relative to the other information protection mechanisms available for protecting personal information in Office 365, Office 365 data loss prevention (DLP) can be used as a measure to detect, warn and block risky sharing activities such as sharing of data containing personal information with an external user.
...
- Integrate with classification & labeling to detect and protect data @ egress

Note that Office 365 DLP will be part of a broader “Microsoft 365 Unified DLP” capability set, as shown in the diagram below.

FIGURE
 
### Supported workloads for Office 365 DLP

With a data loss prevention (DLP) policy in the Office 365 Security & Compliance Center, you can identify, monitor, and automatically protect sensitive information across many locations in Office 365, such as Exchange Online, SharePoint Online, OneDrive for Business, and Microsoft Teams.
...
Your DLP scheme needs to be carefully plotted out based on: (a) business requirements; (b) a risk-based assessment of the organization as highlighted in the article 2. Assess data privacy risks and identify sensitive information; and (c) other information protection and governance mechanisms in place or in planning for data privacy (or other) concerns. A few additional considerations to take into account in this sort of planning are highlighted below.

**Sensitive information types.** First in line will be what you’ve identified as “sensitive data” per your assessment work as outlined in the article 2. Assess data privacy risks and identify sensitive information. DLP policy conditions can currently be predicated on both sensitive information types and retention labels, and Microsoft plans to provide the ability to add Sensitivity Labels as a condition in the future as well.

**Sensitivity Labels.** Though they can’t (currently) be used in DLP policy conditions, certain protection scenarios may be achievable through sensitivity labels alone, since they too can be auto-applied based on sensitive information types and prevent access per Sensitivity Label settings for a given label. If a robust sensitivity labelling scheme is in place (or planned), here’s where DLP should be considered as an augmentative protection mechanism:
...
- DLP policies can be applied to Teams chat and channel messages whereas Sensitivity Labels cannot (since they’re applied to documents and email).
- DLP will be enhanced in the future to include “Endpoint DLP” for endpoint devices, which will provide additional functionality that should be considered in any DLP planning.
...
DLP policies are configured in the Microsoft Compliance admin center and specify the level of protection, sensitive information type (SIT) the policy is looking for, and the target workloads.
...
### Sensitivity labels and DLP – Infographic

The following infographic shows the various types of sensitivity labels working together with DLP to protect data privacy in a highly regulated environment.
As shown, the key steps are as follows:

Prerequisites:

Step 1: Regulatory and otherwise business requirements for data privacy are documented.
Step 2: Target data sources, types and ownership are characterized relative to data privacy concerns.
Step 3: An overall strategy to address requirements and protect/govern data privacy “hotspots” is established.
Step 4: A phased action plan to address the data privacy control strategy is put into place.

Example protection scenario using integrated protection mechanisms:
Step 5A: 
- Sensitivity labels for content are published by an administrator to users/groups of users for manual and/or automatic application to content and email.
- User A applies the labels manually and/or automatically when interacting with content, with encryption and/or other settings applied.
- User A sends a protected email and/or file to user B, a guest user.
Step 5B: 
...
- Protection of internal emails containing sensitive personal information

In addition, Advanced Message Encryption (Advanced OME) helps customers meet compliance obligations that require more flexible controls over external recipients and their access to encrypted emails. With Advanced Message Encryption in Office 365, you can control sensitive emails shared outside the organization with automatic policies. 
...
- If sharing of email with an external party is critical, you can specify expiration parameters and can revoke emails, even if sent to external parties.

### Secure email scenarios comparison (OME vs. Sensitivity labels)

OME and sensitivity labels applied to email (with encryption) have some overlap, so it’s important to understand which scenarios either may apply to, as shown in the table below.

TABLE

Scenari   - Sensitivity Labels	Office 365 Message Encryption
Internal + partners
Securely communicate and collaborate between internal users and trusted partners	Recommend – labels with fully customized classification and protection	Yes – Encrypt only or Do Not Forward protection with no classification
External parties
Securely communicate and collaborate with any external/consumer users	Yes – predefine recipients in label	Recommend – just-in-time protection based on recipients
Internal + partners, with expiration/revocation
Control access of mail and content with internal users and trusted partners with expiration and revocation	Recommend - fully customized protection with access duration, user can manually track and revoke files	No – no revocation or expiration for internal mail
External parties with expiration/revocation
Control access of mail and content with external/consumer users with expiration and revocation	Yes – user can manually track and revoke files	Recommend (E5) – user can revoke mail from Outlook*
Auto-labelling
Organization wants to automatically protect mail/attachments with specific sensitive content and/or specific recipients	Recommend (E5) - Auto-labeling in Exchange service and Outlook clients, augments ETR and DLP policy	Yes - ETR and DLP policy with Encrypt only or Do Not Forward protection

There will also be differences in end-user and admin experiences between these two methods.

## Teams and SharePoint sites access controls for highly regulated data

References:
- Teams for highly regulated data
- SharePoint sites for highly regulated data

For organizations that plan to store personal data (subject to data privacy regulations) in Teams and/or SharePoint Online sites, the articles referenced above provide detailed guidance on a sample set of configuration steps, including the following:

- Identity and device access
- Creation of a private team
- Lockdown site permissions
- Sensitivity label with encryption
- Retention label (as needed)
- DLP policy to block sending of sensitive information outside the organization
...
Refer to the referenced articles for more information.
