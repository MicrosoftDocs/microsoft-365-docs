...
The naming policy is applied to groups that are created across all groups workloads (like Outlook, Microsoft Teams, SharePoint, Planner, Yammer, etc.). It gets applied to both the group name and group alias. It also gets applied when a user creates a group and when group name, alias, description or avatar is edited for an existing group. Even clicking on the update button triggers the naming policy.
