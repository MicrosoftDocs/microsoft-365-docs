...
|multi-factor authentication (MFA) |An authentication method that requires the user to provide two or more verification factors to gain access to a resource such as an application, online account, or a VPN, significantly reducing cyber attacks. |
...
|spam |Sending content to a large number of recipients, usually through email, and often containing malware.  |
|spoofing |When a sender spoofs an email address, they appear to be a user in one of your organization's domains, or a user in an external domain that sends email to your organization. |
