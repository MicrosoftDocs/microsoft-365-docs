...
|OCR   |The number of pages processed for for images (JPEG, JPG, PNG, or BMP); the number of pages processed for PDF, TIF, or TIFF, or the number of embedded images in Teams chats and emails. Each of these counts as one transaction. OCR processing happens again every time the file is edited. If the same image is being sent across different email threads in the tenant, it won't be sent for OCR processing again within the same 24-hour period. |$TBD/transaction| 
