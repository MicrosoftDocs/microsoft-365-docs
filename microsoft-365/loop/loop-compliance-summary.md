...
|Quota  |Loop files in their OneDrive and SharePoint locations follow the quotas of those storage containers.  |One Loop workspace corresponds to one SharePoint Embedded container. Loop SharePoint Embedded containers have no default storage limit, they accrue to the overall storage limits for SharePoint storage quota in the tenant. See [storage quota](/microsoft-365/loop/loop-workspaces-storage-permission#storage-quota) for more information. There's no admin setting available to set storage quotas per SharePoint Embedded container.|
...
For detailed information on existing capabilities in SharePoint Admin Center and PowerShell:
  - Available: [Manage SharePoint Embedded containers in SharePoint Admin Center](/sharepoint/dev/embedded/concepts/admin-exp/consuming-tenant-admin/ctaux) - Learn more about [Loop storage](/microsoft-365/loop/loop-workspaces-storage-permission#storage), which is combined with SharePoint in your tenant.
