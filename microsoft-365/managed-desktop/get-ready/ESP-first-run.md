...
The Enrollment Status Page experience occurs in three phases. For more, see {enrollment status page tracking information THIS IS ON YOUR PERSONAL SHAREPOINT AND IS NOT LINKABLE}

The experience proceeds as follows:

1. The Autopilot experience starts and the user enters their credentials.
2. The device opens the Enrollment Status Page and proceeds through Device Preparation and Device Setup phases. A third phase (Account Setup) is not used at this point by Microsoft Managed Desktop, but is addressed later in the process. The device restarts.
...
- You must target applications in the Test, First, Fast, and Broad groups. Make sure to complete testing with ESP in the test group before you assign them to all groups.
