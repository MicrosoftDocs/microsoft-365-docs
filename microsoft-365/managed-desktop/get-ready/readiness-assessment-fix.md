...
You shouldn't have any existing Autopilot profiles that target assigned or dynamic groups with Microsoft Managed Desktop devices. Microsoft Managed Desktop uses Autopilot to provision new devices. If you have an existing autopilot deployment profile, the setting "Convert all targeted devices to Autopilot" must be set to "No" for the managed desktop readiness test for autopilot to succeed.
