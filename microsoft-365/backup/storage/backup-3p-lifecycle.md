...
2. [Check if the Microsoft 365 Backup Service is enabled in the Consuming Tenant](#step-2-check-if-the-microsoft-365-backup-service-has-been-enabled-in-the-consuming-tenant).
...
After your application is registered, you can always check the state of your application as the Microsoft 365 Backup Storage Controller by executing the [Get serviceApp](https://learn.microsoft.com/graph/api/serviceapp-get?view=graph-rest-beta) API:
```http
...
To check if the Microsoft 365 Backup Service is enabled in the Consuming Tenant, your application needs to execute the [Get backupRestoreRoot](https://learn.microsoft.com/graph/api/backuprestoreroot-get?view=graph-rest-beta) API:
```http
...
If there's no existing Microsoft 365 Backup Storage Controller, then you can immediately activate your application as the Controller. To do this, you execute the [serviceApp: activate](https://learn.microsoft.com/graph/api/serviceapp-activate?view=graph-rest-beta) API:
```http
...
- Your application has read-only access to any existing Protection Policies. Your application won't be able to change or create Protection Policies and/or perform any Restores.
...
Once your application has a status of active, you'll need to enable your Billing Policy in the Consuming Tenant. This step is performed by executing the [backupRestoreRoot: enable](https://learn.microsoft.com/graph/api/backuprestoreroot-enable?view=graph-rest-beta) API:
```http
...
T
o get the state of your application being the Microsoft 365 Backup Storage Controller your application can execute the [Get serviceApp](https://learn.microsoft.com/graph/api/serviceapp-get?view=graph-rest-beta) API:
```http
GET /solutions/backupRestore/serviceApps/{serviceAppId}
```
...
- Your application continues to have access to the existing Protection Policies and is able to change or create Protection Policies and/or perform any Restores (as per your oAuth permission scopes).
...
To deactivate your application from being the Microsoft 365 Backup Service in the Consuming Tenant, your application needs to execute the [serviceApp: deactivate](https://learn.microsoft.com/graph/api/serviceapp-deactivate?view=graph-rest-beta) API:
```http
POST /solutions/backupRestore/serviceApps/{serviceAppId}/deactivate
```
...
To unregister your application from being the Microsoft 365 Backup Storage Controller in the Consuming Tenant, your application needs to execute the [Delete serviceApp](https://learn.microsoft.com/graph/api/backuprestoreroot-delete-serviceapps?view=graph-rest-beta) API:
```http
...
- Your application won't be able access, create, or change any Protection Policies and/or perform any Restores.
