...
action|Enum|The action that will be taken if the indicator will be discovered in the organization. Possible values are: "Alert", "Warn", "Block", "Audit, "BlockAndRemediate", "AlertAndBlock", and "Allowed". **Required**. "GenerateAlert" is an item that must be set to "TRUE" when creating an action with "Audit".
