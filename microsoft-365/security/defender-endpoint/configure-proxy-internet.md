...
Configure a registry-based static proxy for Defender for Endpoint Detection and Response (EDR) sensor to report diagnostic data. Also, Communicate with Defender for Endpoint services, if a computer isn't permitted to connect the Internet.
...
The static proxy is configurable through **Group Policy (GP)**. Under the **Group Policy**, values for both the settings should be set to configure the proxy server to be used for EDR. The group policy is available in Administrative Templates. 
...
Microsoft Defender Antivirus [cloud-delivered protection](cloud-protection-microsoft-defender-antivirus.md) provides near-instant, and automated protection against new and emerging threats. Note, the connectivity is required for [custom indicators](manage-indicators.md) when Defender Antivirus is your active antimalware solution.  For [EDR in block mode](edr-in-block-mode.md) has primary antimalware solution when using a non-Microsoft solution.
...
2. Set it to **Enabled** and define the proxy server. Note, the URL must have either http:// or https://. For supported versions of https://, see [Manage Microsoft Defender Antivirus updates](manage-updates-baselines-microsoft-defender-antivirus.md).
...
Exclude the domains listed in the above table, if a proxy or firewall has enabled the HTTPS scanning (SSL inspection). In your firewall, open all the URLs where the geography column is WW. For rows, the geography column shouldn't be WW. Open the URLs to your specific data location, to verify your data location setting. See [Verify data storage location and update the data retention settings for Microsoft Defender for Endpoint](/microsoft-365/security/defender-endpoint/data-retention-settings).

> [!NOTE]
> Windows devices running with version 1803 or earlier needs `settings-win.data.microsoft.com`.  <br> <br>
 Windows devices running with version 1803 or later needs URLs including v20 in them. For example, Windows device running version with 1803 or later needs `us-v20.events.data.microsoft`.com and it's onboarded to the U.S Data Storage region.
>
If a proxy or firewall is blocking anonymous traffic as Defender for the Endpoint sensor, and it's connecting from system context to make sure anonymous traffic is permitted in the previously listed URLs.
...
The information in the below list of proxy and firewall configuration information is required to communicate with the Log Analytics agent (often referred to as Microsoft Monitoring Agent) for previous versions of Windows, such as Windows 7 SP1, Windows 8.1, and Windows Server 2008 R2*.
...
> [!NOTE]
> *These connectivity requirements apply to the previous Microsoft Defender for Endpoint of Windows Server 2016, and Windows Server 2012 R2 that requires MMA. Instructions to onboard these operating systems with the new unified solution are at [Onboard Windows servers](configure-server-endpoints.md), or at [Server migration scenarios in Microsoft Defender for Endpoint](/microsoft-365/security/defender-endpoint/server-migration).
...
3. Run the TestCloudConnection.exe tool from "C:\Program Files\Microsoft Monitoring Agent\Agent" to validate the connectivity, and get the required URLs for your specific workspace.
...
Verify, the proxy configuration is completed successfully. The WinHTTP can then discover and communicate through the proxy server in your environment, and then the proxy server will allow the traffic to the Defender for Endpoint service URLs.
...
6. Open *MDEClientAnalyzerResult.txt* and verify that you've performed the proxy configuration steps, to enable server discovery and access to the service URLs.
   ...
   The tool checks the connectivity of Defender for Endpoint service URLs. Ensure the Defender for Endpoint client is configured to interact. The toll will then print the results in the *MDEClientAnalyzerResult.txt* file for each URL that can be used to communicate with the Defender for Endpoint services. For example:
...
>
> When the TelemetryProxyServer is set in Registry or via Group Policy, Defender for Endpoint will fall back to direct, if it fails to access the defined proxy.
