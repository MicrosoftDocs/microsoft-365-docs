...
|`36`|LDAP API error|The device was onboarded to Defender for Endpoint but failed to register to AAD due to an AAD Connect misconfiguration. Run the [DSRegTool](/samples/azure-samples/dsregtool/dsregtool) or follow the [troubleshooting instructions](/azure/active-directory/devices/troubleshoot-hybrid-join-windows-legacy) for Windows Server 2012 R2.  |
|`37`|On-premise sync issue|The device was onboarded to Defender for Endpoint but failed to register to AAD due to an AAD Connect misconfiguration. Run the [DSRegTool](/samples/azure-samples/dsregtool/dsregtool) or follow the [troubleshooting instructions](/azure/active-directory/devices/troubleshoot-hybrid-join-windows-legacy) for Windows Server 2012 R2.  |
