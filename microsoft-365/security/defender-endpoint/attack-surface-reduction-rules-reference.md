...
- **Audit**: The state in which the ASR rule is evaluated for the affect it would have on the organization or environment should it be enabled (set to block or warn). The code for this state is 2.
...
Through social engineering or exploits, malware can download and launch payloads, and break out of Adobe Reader. By blocking child processes from being generated by Adobe Reader, malware attempting to use Adobe Reader as an attack vector are prevented from spreading.
