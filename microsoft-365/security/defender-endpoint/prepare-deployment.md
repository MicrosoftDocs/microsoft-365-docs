...
- **Basic permissions management**: Set permissions to either full access or read-only. In the case of basic permissions management users with Globa Administrator or Security Administrator roles in Azure Active Directory have full access. While the Security reader role has read-only access, it does not grant you access to view machines/device inventory.
