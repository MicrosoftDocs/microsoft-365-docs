...
Also, to always-on real-time protection and [on-demand](run-scan-microsoft-defender-antivirus.md) scans, you can set up regular, scheduled scans.
...
>[!NOTE]
>By default, quick scans run on mounted removable devices, such as USB drives. Scanning mapped network drives only works for drives that are mapped on a system level. For more information, see [Configure Microsoft Defender Antivirus scanning options](configure-advanced-scan-types-microsoft-defender-antivirus.md#scanning-mapped-network-drives).
