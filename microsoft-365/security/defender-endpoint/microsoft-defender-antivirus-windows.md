...
3. In the list of results, look at the **AMRunningMode** row.
   ...
   - **EDR Block Mode** means Microsoft Defender Antivirus is running and [Endpoint detection and response (EDR) in block mode](edr-in-block-mode.md), a capability in Microsoft Defender for Endpoint, is enabled. However, you have to check the **ForceDefenderPassiveMode** registry key. If it is equal to 0, it is in normal mode, otherwise, it is in passive mode.
