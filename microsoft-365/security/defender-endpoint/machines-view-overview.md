...
**Windows version** </br> | Filter by the Windows versions you're interested in investigating. If ‘future version’ appears in the Windows version field, it can mean the build:</br> -	is a pre-release version for a future Windows release</br> -	has no version name</br> -	version name is not yet supported </br></br> In all these scenarios, where available, the full OS version can be seen in the device details page.</br></br> (_Computers and mobile only_)
