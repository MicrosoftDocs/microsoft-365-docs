...
|---------|---------|
| Threat Explorer Email Entity Page | This page **unifies** email information that had been scattered across different pages, or views, in Threat Explorer in the past. Investigating email for threats and trends is *centralized*. Header information is accessible on the same page as email authentication status. Likewise, the detonation status for malicious file attachments or URLs can be found on different tabs the same page. The Email entity page empowers admins and security operations teams to understand an email threat and its status, fast, and then act quickly determine handling.  |
...
| [Alert view](/microsoft-365/compliance/alert-policies) | The **View alerts** flyout pane in the Office Security and Compliance center now includes links to the Microsoft 365 security center. Click on the **Open Alert Page** link and the Microsoft 365 security center opens. |
