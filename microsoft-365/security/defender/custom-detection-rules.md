...
> [!IMPORTANT]
> Avoid filtering custom detections using the `Timestamp` column. The `Timestamp` field refers to the time that an event happened and might not always match the time it was uploaded. Data used for custom detections is pre-filtered based on the detection interval.
