...
| **Do we have a list of actions that are supported as a part of Managed Response?** | All device actions found in machineAction resource type and user actions found in <insert link> are in scope for Managed Response for any device and user that is not excluded. |
| **What can the customers customize?** | Customers can configure the extent in which Defender Experts will do the Managed Response actions on their behalf. This can be done by excluding certain devices and users (individually or by groups) from Managed Response, either during onboarding or later on via settings. Refer to <insert link> on how to do this. |
