...
You can try this example query to return information on devices with non compliant antivirus configurations from `DeviceTvmSecureConfigurationAssessment` along with the relevant configuration metadata from the `DeviceTvmSecureConfigurationAssessmentKB` table:
