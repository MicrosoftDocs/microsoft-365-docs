...
1. If you know the filename, use the portal's search bar and type the file's name to open to the file's page directly. Copilot automatically generates the device summary when opening a file page. The results, which shows the overview information by default, are then displayed on the Copilot pane. Select **Show details** to display the file’s information in full.  
