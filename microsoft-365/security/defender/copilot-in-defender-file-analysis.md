...
1. Open a [file page](/microsoft-365/security/defender-endpoint/investigate-files.md#file-page-overview). Copilot automatically generates the device summary when opening a file page. The results, which shows the overview information by default, are then displayed on the Copilot pane. Select **Show details** to display the file’s information in full.  
