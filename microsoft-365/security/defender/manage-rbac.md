---
title: Microsoft 365 Defender Unified role-based access control (URBAC)
...
# Microsoft 365 Defender Unified role-based access control (URBAC)
...
The Microsoft 365 Defender Unified role-based access control (URBAC) model provides a single permissions management experience that provides one central location for administrators to control user permissions across different security solutions.

## What's supported by the Microsoft 365 Defender URBAC model
...
This section provides useful information on what you need to know before you start using Microsoft 365 Defender URBAC.
...
- You must be a Global Administrator or Security Administrator in Azure Active Directory to:
  ...
  - Manage roles and permissions in Microsoft 365 Defender URBAC.

  - Create a custom role that can grant access to security groups or individual users to manage roles and permissions in Microsoft 365 Defender unified RBAC. This will remove the need for Azure Active Directory global roles to manage permissions. To do this you need assign the **Authorization** permission in Microsoft 365 Defender URBAC. For details on how to assign the Authorization permission, see [Create a role to access and manage roles and permissions](../defender/create-custom-rbac-roles.md#create-a-role-to-access-and-manage-roles-and-permissions).

- The Microsoft 365 Defender security solution will continue to respect existing Azure Active Directory global roles when you activate the Microsoft 365 Defender URBAC model for some or all of your workloads i.e. Global Admins will retain assigned admin privileges.

### Migration of existing roles and permissions

The new Microsoft 365 Defender URBAC model provides easy migration of the existing permissions in the individual supported unified RBAC models to the new RBAC model.

All permissions listed within the Microsoft 365 Defender URBAC model align to permissions in the individual RBAC models to ensure backward compatibility. For more information on how the permissions align, see [Map permissions in Microsoft 365 Defender unified role-based access control (RBAC)](compare-rbac-roles.md).

### Activation of the Microsoft 365 Defender URBAC model

You must activate the workloads in Microsoft 365 Defender to use the Microsoft 365 Defender URBAC model. Until activated, Microsoft 365 Defender will continue to respect the existing RBAC models. For more information, see [Activate Microsoft 365 Defender URBAC](activate-defender-rbac.md).
...
## Start using Microsoft 365 Defender URBAC model

Use the following steps as a guide to start using the Microsoft 365 Defender URBAC model:
...
2. **Activate and manage your roles with the Microsoft 365 Defender URBAC model**
   - [Activate Microsoft 365 Defender URBAC](activate-defender-rbac.md)

3. **Learn more about the Microsoft 365 Defender URBAC model**
   - [Microsoft 365 Defender URBAC permissions](custom-permissions-details.md)
