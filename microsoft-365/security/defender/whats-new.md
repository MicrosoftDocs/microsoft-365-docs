...
- (GA) [Asset rule management - Dynamic rules for devices](./configure-asset-rules.md) is now generally available. Dynamic rules can help manage device context by assigning tags and device values automatically based on certain criteria.
- The [DeviceInfo](advanced-hunting-deviceinfo-table.md) table in advanced hunting now also includes the columns `DeviceManualTags` and `DeviceDynamicTags` to surface both manually and dynamically assigned tags related to the device you are investigating.
