...
| Device connectivity | To onboard devices in Defender for Business, those devices must be able to connect to the following URLs: <br/><br/>`enterpriseregistration.windows.net` for registration in Azure Active Directory (Azure AD)<br/><br/>`login.microsoftonline.com` for registration in Azure AD<br/><br/>`*.dm.microsoft.com` The wildcard (*) supports the cloud-service endpoints that are used for enrollment, check-in, and reporting, and can change as the service scales. | 
