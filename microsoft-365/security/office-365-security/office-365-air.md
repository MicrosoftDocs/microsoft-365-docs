...
1. An automated investigation is initiated in one of the following ways:
   ...
   - A security analyst [starts an automated investigation](automated-investigation-response-office.md#example-a-security-administrator-triggers-an-investigation-from-threat-explorer) while using [Explorer](threat-explorer.md).  Note that these admin triggered investigations will also have an alert, but it's the Explorer action generating the alert for alert and investigation consistency.
