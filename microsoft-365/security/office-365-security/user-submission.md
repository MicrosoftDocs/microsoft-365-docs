...
- **Send the reported messages to** section: Select one of the following options:
  ...
  - **Microsoft**: The user reports go directly to Microsoft for analysis. Only the metadata such as sender, recipient, reported by and the message details from the user reports are provided to the tenant admin via the M365 Security Center.
