...
|Threats Found |The automated investigation found issues, but there are no specific remediation actions to resolve those issues.<br/><br/> This can occur when some type of user activity was identified but no cleanup actions are available. Examples include a DLP event, an email sending anomaly, sent malware, or sent phish, for which the investigation found no malicious URLs, files, or email messages to remediate, and no mailbox activity to fix (forwarding rules or delegation). <br/><br/>**TIP**: If you suspect something was missed (such as a false negative), you can investigate and take action using [Threat Explorer](https://docs.microsoft.com/microsoft-365/security/office-365-security/threat-explorer)​. |
