...
|**Advanced phishing thresholds**|Defines the level of settings for how phishing messages are handled.  <br/> **Standard** Email suspected to be phish is handled in the standard way.  <br/> **Aggressive** System handles emails suspected to be phish with a high degree of confidence the same way as those, suspected with very high degree of confidence.  <br/> **More aggressive** System handles emails suspected to be phish with a medium or high degree of confidence the same way as those, suspected with very high degree of confidence.  <br/> **Most aggressive** System handles emails suspected to be phish with a low, medium or high degree of confidence the same way as those, suspected with very high degree of confidence.|When you want to be more aggressive in the treatment of potentially phishing messages within Office 365. For example, messages with a very high probability of being phish will have the most aggressive actions taken on them while messages with a low probability have less aggressive actions taken on them. This setting also impacts other parts of the filtering system that combine signals together. This doesn't necessarily mean different actions are implemented.  Essentially you set the probability of mail being phish, to determine the (same) designated action. The chance of moving good messages increases as the level of settings increases.|
