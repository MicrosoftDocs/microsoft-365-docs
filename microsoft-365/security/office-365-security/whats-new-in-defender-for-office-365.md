...
- Bulk complaint level (BCL) is now available in the EmailEvents table in advanced hunting with numeric BCL values ranging from 0 to 9. A higher BCL score indicates that bulk messages are more likely to generate complaints and are more likely to be spam.
