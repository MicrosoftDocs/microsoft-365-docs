...
The **User reported messages** report shows information about email messages that users have reported as junk, phishing attempts, or good mail by using the [built-in Report button in Outlook on the web](zzz-built-in-owa-report.md) or the [Microsoft Report Message or Report Phishing add-ins](enable-the-report-message-add-in.md).
