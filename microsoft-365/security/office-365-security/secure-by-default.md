...
## More on why we are doing this
The spirit of being secure by default is that we are taking the action that you would be taking if you knew that this was malicious even if there was an allow in place. This is the same action we have been taking on malware for a long time and we moved high confidence phish in parity with that. We have data to support that the false positive rate is very low here and if a false positive was to happen, then it could be resolved with an admin submission. We also have the data to support that the tenant anti-spam policy and the user safe sender allows were too broad and causing more harm than good. Another way to put it is that as a security service, we are taking the steps for you to prevent your users from getting compromised. In addition, this is not a full reject in that the items would take the action that is set in your anti-spam policy, which is either Quarantine, Move to Junk folder, or Redirect to an email address.
