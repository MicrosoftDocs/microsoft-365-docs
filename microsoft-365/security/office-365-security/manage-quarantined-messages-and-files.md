...
2. Verify that the **Email** tab is selected.
...
- **Release email**: In the flyout pane that appears, choose the following options:
...
- **View message headers**: Choose this link to see the message header text. To analyze the header fields and values in depth, click **Copy message header** and then click **Microsoft Message Header Analyzer** to open the Message Header Analyzer in a new tab. Paste the message header onto the page in the Message Header Analyzer section, and choose **Analyze headers**:

The following options are available after you click ![More options icon](../../media/m365-cc-sc-more-actions-icon.png):
...
- **Download safe email**: In the flyout pane that appears, select **I understand the risks from downloading this message** to save a local copy of the message in .eml format.
...
- **Submit message**: Select one fof the following options:
  - **Report clean**
  - **Report phishing**
  - **Report malware**
  - **Report spam**

  Click **OK** in the confirmation that appears.
...
When you select multiple quarantined messages in the list (up to 100), the **Bulk actions** drop down list appears where you can take the following actions:
...
3. You can sort the results by clicking on an available column header. Click **Customize columns** to show a maximum of seven columns. The default columns are marked with an asterisk (<sup>\*</sup>):
...
4. To filter the results, click **Filter**. The available filters are:
   ...
   - **Time received** (**Start time** and **End time**)
   - **Expires**  (**Start time** and **End time**)
