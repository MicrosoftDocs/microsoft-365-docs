...
>
> If you are using a domain instead of the IP/IP Range in the sending infrastructure, it needs to match the PTR record for the connecting IP in the "Authentication-Results" header. You can determine the PTR by using `ping -a <IP address>`. We also recommend using the PTR Organization Domain, which means that if the PTR resolves to "smtp.inbound.contoso.com", you should use "contoso.com" as the sending infrastructure.
