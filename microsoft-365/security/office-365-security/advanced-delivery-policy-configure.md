...
>
> At present, the advanced delivery policy for third-party phishing simulations does not support simulations within the same organization (DIR:INT), especially when emails are routed to Microsoft 365 via an Exchange Server gateway with Hybrid mail flow. To work around this, you can create a unique [send connector](https://learn.microsoft.com/en-us/exchange/mail-flow/mail-routing/connector-selection?view=exchserver-2019) that does not authenticate these emails as internal or configure your simulation to route directly to your EOP MX record (contoso-com.mail.protection.outlook.com), bypassing the Exchange Server infrastructure. Another option is to set intra-organization message scanning to None in the [anti-spam policies](https://learn.microsoft.com/en-us/microsoft-365/security/office-365-security/anti-spam-policies-configure?view=o365-worldwide#use-the-microsoft-defender-portal-to-create-anti-spam-policies), but this is not recommended as it will affect other email scanning besides the third-party simulations.
> 
