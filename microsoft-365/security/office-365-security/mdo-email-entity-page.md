...
Security teams can take email actions like soft delete and hard delete, move to junk, move to inbox, trigger an investigation, submit to Microsoft for review in line, and et cetera. **Tenant level block** actions like file and URL or sender can also be triggered from the Email entity page.
...
> [!NOTE]
In the same ** Take actions" We are adding the ability to take multiple actions together. You can take email remediation actions, create submissions, tenant level block actions (block sender/domain/**file/URLs**), investigative actions and proposed remediation from the **same panel**. Actions are now contextual and grouped together depending on the **latest location of the email message**. 
...
> There is no change to the action permission. 
