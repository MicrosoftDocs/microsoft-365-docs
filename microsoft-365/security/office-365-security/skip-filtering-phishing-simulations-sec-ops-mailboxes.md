...
> [!NOTE]
> Enhanced Filtering for Connectors doesn't work for third-party phishing simulations in complex email routing scenarios (for example, email from the internet is routed to Microsoft 365, then to an on-premises environment or third-party security service, and then back to Microsoft 365. EOP can't identify the true IP address of the message source in those complex scenarios. Don't try to work around this limitation by adding the IP addresses of the on-premises or third-party sending infrastructure to the third-party phishing simulation. Doing so effectively bypasses spam filterting for any internet sender who impersonates the domain that's identified in the third-party phishing simulation.
>
