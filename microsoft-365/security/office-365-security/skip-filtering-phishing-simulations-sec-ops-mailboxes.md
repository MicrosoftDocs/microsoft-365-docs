...
> [!NOTE]
> In case of complex mail routing scenario with MX record pointed to Microsoft 365 (for example,**Internet -> MX Pointed to EOP -> on-premises/3rd party -> EXO**) , enabling
[Enhanced Filtering for Connectors](/Exchange/mail-flow-best-practices/use-connectors-to-configure-mail-flow/enhanced-filtering-for-connectors) and configuring advanced delivery policy are not supported as EOP cannot identify the IP of the true source of the email, making it impossible to trigger the PhishSimOverrideRule scoped to sending IP and P1/DKIM domain.Do not attempt to work around this complex routing limitation by adding On-Premises/3rd Party infrastructure IPs to PhishSimOverrideRule. This would effectively bypass spam filtering for any Internet Sender impersonating the P1 domain used for the Phish Simulation.
