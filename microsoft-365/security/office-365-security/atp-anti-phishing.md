...
ATP anti-phishing checks incoming messages for indicators that the message may be phishing. Whenever a user is covered by an ATP policy (Safe Attachments, Safe Links or ATP anti-phishing) the incoming message is evaluated by multiple machine learning models. The message is analyzed to determine if the policy applies to the message. Based on the configuration of the policy, the appropriate action is taken on the message.
...
ATP anti-phishing policies can be applied to a specific set of people or groups in your organization, or to an entire domain or all of your custom domains. To learn more, see [Set up anti-phishing policies in Office 365](set-up-anti-phishing-policies.md).
