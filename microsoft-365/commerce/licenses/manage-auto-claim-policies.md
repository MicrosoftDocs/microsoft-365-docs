...
By default, the auto-claim policy feature is turned off. You must turn
the feature on before you can use it. After you turn on the feature, you
can create an auto-claim policy.

### To turn auto-claim policies on:
...
### To turn auto-claim policies off:
...
the policy. If the primary product is out of licenses, the backup
product is used to assign licenses to users. You can add multiple backup
products and [change the order in which they are used](#change-the-assigning-order-for-backup-products). To learn more,
...
When you create a policy, you add a product to it, and licenses are then
automatically assigned to users from that pool of licenses. You can add
or remove products to an auto-claim policy at any time. If you already
...
and services are included when a user is automatically assigned a
...
order of the products that are used to assign licenses when users log in
to the app.
