---
title: Set or adjust settings preferences for Scheduler for Microsoft 365 Overview
ms.author: shivb
author: shivbijlani
manager: charlle
audience: Admin
...
Scheduler uses several Outlook preferences to schedule a meeting for an organizer. Any changes to the preference settings in Outlook clients will affect how Scheduler handles subsequent requests sent to Cortana. For instance, if an organizer changes the time zone preference on the Settings page in Outlook Web, all subsequent requests by the organizer will default to the new time zone value.
...
- **Default meeting duration**. If the organizer does not specify the desired meeting duration in the request, Scheduler will use the preferred meeting duration for the request. This setting is only available in the Windows Outlook client.
   ...
   1. Select **File** > **Options** to see the Outlook Options window.

   2. Select **Calendar** from the list on the left in the window.
