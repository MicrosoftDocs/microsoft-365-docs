...
In addition to Advanced Audit's one year retention capabilities, we have also released the capability to retain audit logs for 10 years. The 10-year retention of audit logs helps support long running investigations and respond to regulatory, legal, and internal obligations.

> [!NOTE]
> Retaining audit logs for 10 years will require an additional per-user add-on license. Once this license is assigned to a user, and an appropriate 10-year log retention policy is set for that user, Audit logs covered by that policy will start to be retained for the 10-year period. This policy is not retroactive and can not retain Audit logs generated previously for the 10-year period. For more information, see the [FAQs for Advanced Audit](#faqs-for-advanced-audit) section in this article.
...
For eligible customers and users that are assigned the appropriate E5/A5/G5 license, there is no action needed to generate access to crucial auditing events, except for the SearchQueryInitiatedExchange and SearchQueryInitiatedSharePoint events (as described earlier in this article).
