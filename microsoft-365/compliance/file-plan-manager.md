...
|IsRecordUnlockedAsDefault|String|No|This property specifies whether the item marked as a record is [unlocked](record-versioning.md) when the label is applied. </br> </br> Do not specify this property unless **IsRecordLabel** property is set to **TRUE**. The property **Regulatory** can't be set to **TRUE**.|
