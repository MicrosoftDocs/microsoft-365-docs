...
> [!NOTE]
> SharePoint sites have a requirement for retention settings that the wizard can't check, which is that they must be indexed. However, if documents in SharePoint libraries are configured to not appear in search results, this configuration doesn't exclude them from the retention settings.
