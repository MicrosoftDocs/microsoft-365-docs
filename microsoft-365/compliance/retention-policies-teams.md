...
You can use a retention policy to retain chats and channel messages in Teams. Teams chats are stored in a hidden folder (**Conversation History**\**Teams Chat**) in the mailbox of each user included in the chat, and Teams channel messages are stored in a similar hidden folder in the group mailbox for the team.
...
After a retention policy is configured for chat and channel messages, a timer job from the Exchange service periodically evaluates items in the Teams Chat folder and the SubstrateHolds folder. The SubstrateHolds folder is another hidden folder that's in every user or group mailbox that stores copies of edited or deleted messages that must be retained. When an item has expired its retention period in the Teams Chat folder, it's moved to the SubstrateHolds folder. The timer job can take up to seven days to run.
...
1. **If a chat or channel message is edited or deleted** by the user during the retention period, the message is copied (if edited) or moved (if deleted) to the  SubstrateHolds folder. The message is stored there until the retention period expires and then the message is permanently deleted.

2. **If a chat or channel message is not deleted** and for current messages after editing during the retention period, the message is moved to the SubstrateHolds folder after the retention period expires. The message in the SubstrateHolds folder is then permanently deleted within one day. 

> [!NOTE]
> Messages in the SubstrateHolds folder are searchable by eDiscovery tools. After a message is deleted from there, it won't be returned in an eDiscovery search.

When the retention policy is retain-only, or delete-only, the content's paths are variations of retain and delete.

### Content paths for retain-only retention policy

1. **If a chat or channel message is edited or deleted** during the retention period: A copy of the original message is created in the SubstrateHolds folder and retained until the end of the retention period, when the copy in the SubstrateHolds folder is deleted after the item expires. 

2. **If the item is not modified or deleted** and for current messages after modification during the retention period: Nothing happens before and after the retention period; the message remains in its original location.

### Content paths for delete-only retention policy

1. **If the message is not deleted** during the retention period: At the end of the retention period, the message is moved to the SubstrateHolds folder where it is permanently deleted within one day. 

2. **If the item is deleted by the user** during the period, the item is immediately moved to the SubstrateHolds folder where it is permanently deleted within one day..
