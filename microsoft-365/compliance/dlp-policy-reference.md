...
- Content contains
- (preview) Document or attachment is password protected (.pdf, Office files and Symantec PGP encrypted files are fully supported).
 Note: This predicate currently doesn’t detect DRM encrypted or permission protected files. 
- (preview) Content is not labeled (.pdf, Office files are fully supported).
 This predicate detects content that doesn't have a sensitivity label applied. To help ensure only supported file types are detected, we recommend pairing this condition with the ‘File extension is’ or ‘File type is’ condition.
