...
> [!NOTE]
> As previously stated, the Managed Folder Assistant processes mailboxes at least once every 7 days. So it's possible that a mailbox can be processed by the Managed Folder Assistant more frequently than once every 7 days. Admins can't predict the next time a mailbox is processed by the Managed Folder Assistant, which why you can manually run it. However, if you want to temporarily prevent the Managed Folder Assistant from applying the new retention settings to a mailbox, you can run the `Set-Mailbox -ElcProcessingDisabled $true` command to temporarily disable the the Managed Folder Assistant from from processing a mailbox. To re-enable the Managed Folder Assistant to process the mailbox, run the `Set-Mailbox -ElcProcessingDisabled $false` command.
