...
|**Administrative action submitted by an Administrator**|Manual email actions can be taken on email entities using various surfaces today like explorer, advanced hunting or through custom detection. As the remediation gets kicked-off, it generates an alert. This alert shows up in the alerts queue with the name " Administrative action submitted by an Administrator" suggesting that an admin took the action of remediating an entity. It presents details like action type, supporting investigation link, time etc. It works really well to know every time a sensitive action like remediation is performed on entities. This policy has an **Informational** severity setting.|Suspicious activity |Yes|E5/ Microsoft Defender for Office 365 P2 add-on subscription|
> [!NOTE]
