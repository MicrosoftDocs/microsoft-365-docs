...
- seo-marvel-apr2020
- admindeeplinkMAC
description: Learn how to use the Import service in the Microsoft Purview portal to bulk-import email data (PST files) to user mailboxes.
---
...
You can use the Import service in the <a href="https://go.microsoft.com/fwlink/p/?linkid=2077149" target="_blank">Microsoft Purview portal</a> to quickly bulk-import PST files to Exchange Online mailboxes in your organization. There are two ways you can import PST files to Microsoft 365:
...
4. **Create a PST import job** - The next step is to create a PST import job on the **Import PST files** page in the Microsoft Purview portal and submit the PST import mapping file created in the previous step. For network upload (because the PST files have been uploaded to Azure) Microsoft 365 analyzes the data in the PST files and then gives you an opportunity to set filters that control what data actually gets imported to the mailboxes specified in the PST import mapping file.
...
6. **Start the PST import job** - After the import job is started, Microsoft 365 uses the information in the PST import mapping file to import the PST files from the Azure Storage location to user mailboxes. Status information about the import job (including information about each PST file being imported) is displayed on the **Import PST files** page in the Microsoft Purview portal. When the import job is finished, the status for the job is set to **Complete**.
...
- Importing email data to Microsoft 365 helps address compliance needs of your organization by letting you:
  ...
  - Use [retention policies](retention.md) in the Microsoft Purview portal to control how long mailbox content is retained, and then delete content after the retention period expires.
...
Additionally, to create import jobs in the Microsoft Purview portal, one of the following must be true:
...
When you use the network upload method to import PST files, you upload them to an Azure blob container named `ingestiondata`. If there are no import jobs in progress on the **Import PST files** page in the Microsoft Purview portal), then all PST files in the `ingestiondata` container in Azure are deleted 30 days after the most recent import job was created in the Microsoft Purview portal. That also means you have to create a new import job in the Microsoft Purview portal (described in Step 5 in the network upload instructions) within 30 days of uploading PST files to Azure.

This also means that after PST files are deleted from the Azure Storage area, they're no longer displayed in the list of files for a completed import job in the Microsoft Purview portal. Although an import job might still be listed on the **Import PST files** page in the Microsoft Purview portal, the list of PST files might be empty when you view the details of older import jobs.
...
All PST files in the Azure Storage location for your organization (in blob container named `ingestiondata`), are deleted 30 days after the most recent import job was created on the **Import PST files** page in the Microsoft Purview portal.
