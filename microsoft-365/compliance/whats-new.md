...
- **In preview**: For mandatory labeling, Outlook for Android in the Beta Channel supports a setting from the Intune App Software Development Kit (SDK) to [prompt users to select a sensitivity label when they first compose an email](sensitivity-labels-office-apps.md#for-outlook-mobile-change-when-users-are-prompted-for-a-label) instead of when they send it.
