...
- Sensitivity labels that are configured for [automatic labeling](apply-sensitivity-label-automatically.md#how-to-configure-auto-labeling-for-office-apps) work for Office on the web when the label settings for conditions are for sensitive information types only. Automatic labeling doesn't work for Office on the web when the conditions include trainable classifiers.
