...
- SharePoint and OneDrive can't process labeled and encrypted files that have PowerQuery data, data stored by custom add-ins, or custom XML parts that have custom XML parts such as Cover Page Properties, content type schemas, custom Document Information Panel, and Custom XSN. For these files, either apply a label without encryption so that they can be opened in Office on the web, or instruct users to open the files in their desktop apps.
