...
- If a labeled document is uploaded to SharePoint and the document has been encrypted by using an account from a service principal name, the document can't be opened in Office on the web. Example scenarios include Microsoft Cloud App Security labeling and encryting a file, and a labeled and encrypted file sent to Teams by email.
