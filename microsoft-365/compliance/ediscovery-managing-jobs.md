...
| Reindex data sources | When you add a data source to a case, all partially indexed items in the selected data sources are reindexed by a process called *Advanced indexing*. This job is also triggered when you select **Update index** on the **Processing** tab of a case, and when you update the index for a data source on the data source properties flyout page. For more information, see [Advanced indexing of custodian data](ediscovery-indexing-custodian-data.md). |
