...
|Family ID|FamilyId|Family_ID|Groups together all items for email - this includes the message and all attachments / extracted items.|
...
|Group Id|Group Id|Group_ID|Groups together all items for email and documents; for email, this includes the message and all attachments / extracted items; for documents, this includes the document and any embedded items.|
...
|Set ID||Set_ID|Documents of similar content (ND_set) or emails within the same email thread (Email_set) share the same Set_ID|
