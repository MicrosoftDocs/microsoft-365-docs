...
**Similar job title**: Users with a combination of organization distance and similar job titles. For example, a user with an account manager title with a similar role designation as a sales manager in the same organization would be identified as similar job title. This option requires that your organization uses Azure AD to maintain organization hierarchy, role designations, and job titles. If you do not have Azure AD configured for organization structure and job titles, then Insider risk management identifies peer groups based on common SharePoint sites.
