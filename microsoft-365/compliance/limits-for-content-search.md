...
|Rate at which search results from mailboxes and sites are exported to a Microsoft-provided Azure Storage location or downloaded from that location to a local drive. |Minimum of 1 GB per hour for a single mailbox or site, 10 GB per hour for search results from more than 10 mailboxes |
|||
