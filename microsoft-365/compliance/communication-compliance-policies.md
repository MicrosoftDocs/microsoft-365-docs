...
|:-----|:-----|:-----|
| **Inappropriate text** | Detect potentially inappropriate text | - Locations: Exchange Online, Microsoft Teams, Yammer <br> - Direction: Inbound, Outbound, Internal <br> - Review Percentage: 100% <br> - Conditions: Threat, Discrimination, and Targeted harassment classifiers |
| **Inappropriate images** | Detect potentially inappropriate images | - Locations: Exchange Online, Microsoft Teams, Yammer <br> - Direction: Inbound, Outbound, Internal <br> - Review Percentage: 100% <br> - Conditions: Adult and Racy image classifiers |
| **Sensitive information** | Check for sensitive info | - Locations: Exchange Online, Microsoft Teams, Yammer <br> - Direction: Inbound, Outbound, Internal <br> - Review Percentage: 10% <br> - Conditions: Sensitive information, out-of-the-box content patterns, and types, custom dictionary option, attachments larger than 1 MB |
| **Regulatory compliance** | Check for regulatory compliance | - Locations: Exchange Online, Microsoft Teams, Yammer <br> - Direction: Inbound, Outbound <br> - Review Percentage: 10% <br> - Conditions: custom dictionary option, attachments larger than 1 MB |
| **Conflict of interest** | Check for conflict of interest | - Locations: Exchange Online, Microsoft Teams, Yammer <br> - Direction: Internal <br> - Review Percentage: 100% <br> - Conditions: None |

Communications are analyzed every 24 hours from the time that a policy is created. For example, if you create a new content policy at 11:00 AM, the policy will gather communication compliance signals every 24 hours at 11:00 AM daily. Editing a policy doesn't change this time. To view the last run date and time for a policy, navigate to the *Last policy scan* column on the **Policy** page. After creating a new policy, it may take up to 24 hours to view the first policy check date and time. The date and time of the last check are converted to the time zone of your local system.

## User-reported messages policy

> [!NOTE]
> Availability for user-reported messages for organizations licensed and using [communication compliance](/microsoft-365/compliance/communication-compliance-configure#subscriptions-and-licensing) and Microsoft Teams started in May 2022. This feature, available since August 31, 2022, is for all organizations licensed and using communication compliance before July 2022. For organizations starting to use communication compliance after July 2022, user-reported messages policy availability may take up to 30 days from the date of your licensing and first use of communication compliance.

As part of a layered defense to detect and remediate potentially inappropriate messages in your organization, you can supplement communication compliance policies with user-reported messages in Microsoft Teams. This feature empowers users in your organization to self-report potentially inappropriate internal personal and group chat messages, such as potentially harassing or threatening language, sharing of adult content, and sharing of sensitive or confidential information, to help foster a safe and compliant work environment.

Enabled by default in the [Microsoft Teams admin center](/microsoftteams/manage-teams-in-modern-portal), the *Report a concern* option in Teams messages allows users in your organization to submit potentially inappropriate internal personal and group chat messages for review by communication compliance reviewers for the policy. These messages are supported by a default system policy that supports reporting messages in Teams group and private chats.

![Communication compliance Report a concern.](../media/communication-compliance-report-a-concern-full-menu.png)

When a user submits a Microsoft Teams chat message for review, the message is copied to the user-reported message policy. Reported messages initially remain visible to all chat members and there isn't any notification to chat members or the submitter that a message has been reported in channel, private, or group chats. A user can't report the same message more than once and the message remains visible to all users included in the chat session during the policy review process.

During the review process, communication compliance reviewers can perform all the standard [remediation actions](/microsoft-365/compliance/communication-compliance-investigate-remediate#step-3-decide-on-a-remediation-action) on the message, including removing messages from the Teams chat. Depending on how messages are remediated, the message sender and recipients will see different [notification messages](/microsoftteams/communication-compliance#act-on-inappropriate-messages-in-microsoft-teams) in Teams chats after the review.
...
To customize reviewers for chat messages submitted as user-reported messages, complete the following steps:
...
- **Policy storage limit reached**: Active communication compliance policies have message storage limits. When the storage limit for a policy is reached, the policy is automatically deactivated. Organizations that need to continue to detect, capture, and act on potentially inappropriate messages covered by the deactivated policy can quickly create a new policy with an identical configuration.
- **Detect and review potentially inappropriate messages for different groups of users**: Some organizations may prefer to create multiple policies with the same configuration but include different in-scope users and different reviewers for each policy.
...
To copy a policy and create a new policy:
...
Communications are checked every hour from the time policies are created. For example, if you create a potentially inappropriate content policy at 11:00 AM, the policy will gather communication compliance signals every hour starting from when the policy was created. Editing a policy doesn't change this time. To view the last check date and time for a policy, navigate to the *Last policy scan* column on the **Policy** page. After creating a new policy, it may take up to an hour to view the first policy's date and time. The date and time of the last check are converted to the time zone of your local system.
...
To identify a policy older than July 31, 2022, review *Last policy scan* column on the **Policy** page. Older policies will display a full date for the check while policies created after July 31, 2022 will display *1 hour ago* for the check. Another option to reduce latency is waiting until after December 31, 2022 for your existing policies to be automatically migrated to the new detection criteria.
...
To manage policies approaching the storage and message limits, consider making a copy of the policy to maintain coverage continuity. You can also take the following actions to help minimize current policy storage size and message counts:
...
- If a policy has reached the storage or message limits and has been deactivated, make a copy of the policy to continue detecting and taking action for the same conditions and users.

## Policy settings

### Users

You can select **All users** or determine specific users in a communication compliance policy. Selecting **All users** applies the policy to all users and all groups that any user is included in as a member. Defining specific users applies the policy to the defined users and any groups the defined users are included in as a member.
...
You have the option of including sensitive information types as part of your communication compliance policy. Sensitive information types are either pre-defined or custom data types that can help identify and protect credit card numbers, bank account numbers, passport numbers, and so on. As part of [Learn about Microsoft Purview Data Loss Prevention](/microsoft-365/compliance/dlp-learn-about-dlp), the sensitive information configuration can use patterns, character proximity, confidence levels, and even custom data types to help identify and flag content that may be sensitive. The default sensitive information types are:
...
[Built-in trainable and global classifiers](/microsoft-365/compliance/classifier-learn-about) check sent or received messages across all communication channels in your organization for different types of compliance issues. Classifiers use a combination of artificial intelligence and keywords to identify language in messages likely to violate anti-harassment policies.

Communication compliance uses built-in trainable and global classifiers to analyze communications for terms, images, and sentiment for the following types of language and content:

- **Adult images**: Checks for images that are potentially sexually explicit in nature.
- **Customer Complaints**: Checks for feedback and complaints made about your organization's products or services.
- **Discrimination**: Checks for potentially explicit discriminatory language and is particularly sensitive to discriminatory language against the African American/Black communities when compared to other communities.
- **Gory images**: Checks for images that depict violence and gore.
- **Harassment**: Checks for potentially offensive conduct targeting people regarding race, color, religion, national origin.
- **Profanity**: Checks for potentially profane expressions that embarrass most people.
- **Racy images**: Checks for images that are potentially sexually suggestive in nature, but contain less explicit content than images deemed Adult.
- **Threat**: Checks for potential threats to commit violence or physical harm to a person or property.

> [!NOTE]
> Policies using classifiers will inspect and evaluate messages with a word count of six or greater. Messages containing less than six words aren't evaluated in policies using classifiers. To identify and take action on shorter messages containing potentially inappropriate content, we recommend including a custom keyword dictionary to communication compliance policies detecting this type of content.

### Optical character recognition (OCR)

Configure built-in or custom communication compliance policies to analyze and identify printed or handwritten text from images that may be potentially inappropriate in your organization. Integrated [Azure Cognitive Services and OCR support](/azure/cognitive-services/computer-vision/overview-ocr) for identifying text in images help analysts and investigators detect and act on instances where potentially inappropriate conduct may be missed in communications that is primarily non-textual.

You can enable optical character recognition (OCR) in new policies from templates, custom policies, or update existing policies to expand support for processing embedded images and attachments. When enabled in a policy created from a policy template, automatic analyses is supported for embedded or attached images in email and Microsoft Teams chat messages. For images embedded in document files, OCR isn't supported. For custom policies, one or more conditional settings associated with keywords, built-in classifiers, or sensitive info types must be configured in the policy to enable the selection of OCR documents.

Images from 50 KB to 4 MB in the following image formats are analyzed and processed:
...
> [!NOTE]
> Checking for and extraction of embedded and attached .pdf images is currently supported only for email messages.
...
#### Match words and phrases to emails or attachments
