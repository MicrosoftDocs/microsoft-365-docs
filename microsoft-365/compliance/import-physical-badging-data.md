...
description: "Administrators can set up a data connector to import data from their organization's physical badging system to Microsoft 365. This lets you use this data in insider risk management policies to help you detect access to your physical building by specific users that may indicate a possible internal threat to your organization."
---


# Set up a connector to import physical badging data (preview)

You can set up a data connector in the Microsoft 365 compliance center to import physical badging data, such as employee’s raw physical access events or any physical access alarms generated by your organization's badging system. physical badging data can then be used by the Microsoft 365 [insider risk management solution](insider-risk-management.md) to help protect your organization from malicious activity or data theft inside your organization. Setting up a physical badging connector consists of the following tasks:

- Creating an app in Azure Active Directory to access an API endpoint that accepts a JSON payload that contains physical badging data.

- Creating the JSON payload with a schema defined by connector.
...
- Optionally, schedule the script to run automatically.
...
Make sure you generate the JSON file as per the following schema definition:

![](c:\\GitHub\\microsoft-365-docs-pr\\microsoft-365\\compliance/media/image1.png)

Here are descriptions of the schema properties:

|**Property**|**Description**|**Data type**|
|:-----------|:--------------|:------------|
|User UPN|An employee can have multiple digital identities across the systems. The input needs to have the AAD ID already resolved by the source system. |Email address|
|AssetName|This is the friendly name of the physical asset or access point.|Alphanumeric string|
|AssetID|The reference ID of the physical access point.| Alphanumeric string|
|Time|The time stamp of access.|Time (UTC)|
|AccessStatus|Success or Failed| String|
|||
...
4. On the **Authentication credentials** page, do the following and then click **Next**:
   ...
   2. Download the sample schema for your refence to create JSON payload
...
6. A status page is displayed that confirms the connector was created. This page also contains the job ID. You can copy it from this page or from the flyout page for the connector.

7. Status page also contains a link to the script. Please refer this script to understand how to post the JSON file to the API endpoint.

8. Click **Done**.

   The new connector is displayed in the list on the **Connectors** tab.

9. Click the physical badging connector that you just created to display the flyout page, which contains properties and other information about the connector.

   ![](c:\\GitHub\\microsoft-365-docs-pr\\microsoft-365\\compliance/media/image2.png)
...
3. Under **Progress**, click the **Download log** link to open (or save) the status log for the connector. This log contains information about each time the script runs and uploads the data from the CSV file to the Microsoft cloud.
   ...
   ![HR connector log file displays number rows from CSV file that were uploaded](c:\\GitHub\\microsoft-365-docs-pr\\microsoft-365\\compliance/media/image3.png)
...
7. Select the **Actions** tab, click **New**, and then do the following things:
   ...
   ![Action settings to create a new scheduled task for the HR connector script](c:\\GitHub\\microsoft-365-docs-pr\\microsoft-365\\compliance/media/image4.png)
...
8. In the **Create Task** window, click **Ok** to save the scheduled task. You might be prompted to enter your user account credentials.
   ...
   ![The new task is displayed in the Task Scheduler Library](c:\\GitHub\\microsoft-365-docs-pr\\microsoft-365\\compliance/media/image5.png)
