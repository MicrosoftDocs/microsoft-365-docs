...
|`DLP-group@contoso.com` can’t be used as a value for the Shared By condition because it’s a distribution group or mail-enabled security group. Please use Shared by Member of predicate to detect activities by members of certain groups.     |Transport rules allow groups to be used in the `sender is` condition but unified DLP does not allow it.         | Update the transport rule to remove all group email addresses from the `sender is` condition and add the group to the `sender is a member of` condition if required. 
Retry the migration for the impacted policy|
|Could not find recipient 'DLP-group@contoso.com'. If newly created please retry the operation after sometime. If deleted or expired please reset with valid values and try again.     |It is likely that the group address used in 'sender is a member of' or 'recipient is a member of' condition is expired or invalid.         | - Remove/replace all the invalid group email addresses in the transport rule in Exchange Admin Center </br> - Retry the migration for impacted policy.|
|The value specified in 'FromMemberOf' predicate must be mail enabled security group.     |Transport rules allow individual users to be used in "sender is a member of" condition but unified DLP does not allow it.         | - Update the transport rule to remove all individual user email addresses from "sender is a member of" condition and add the users to "sender is" condition if required. </br> - Retry the migration for impacted policy.|
|The value specified in 'SentToMemberOf' predicate must be mail enabled security group.    |Transport rules allow individual users to be used under "recipient is a member of" condition but unified DLP does not allow it         | - Update the transport rule to remove all individual user email addresses from "recipient is a member of" condition and add the users to "recipient is" condition if required. </br> - Retry the migration for impacted policy.|
