...
> [!NOTE]
> Retention policies with [preservation lock](retention-preservation-lock) enabled can have the retention period extended, but not decreased or removed.
...
Holds associated with eDiscovery cases are indefinite holds, which means there's no hold duration that can be changed. Items are held forever or until the [hold is removed](create-ediscovery-holds#removing-content-locations-from-an-ediscovery-hold) or the case is closed.
