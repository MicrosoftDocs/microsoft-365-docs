...
3. Run the following command the first time that you use the WAImportExport.exe to copy PST files to a hard drive.
    ...
    | `/blobtype:` <br/> |Specifies the type of blobs in the Azure Storage area to import the PST files to. For importing PST files, use the value **BlockBob**. This parameter is required.   <br/> | `/blobtype:BlockBlob` <br/> |
