...
|"  "|"fair value" <p> subject:"Quarterly Financials"|Use double quotation marks ("  ") to search for an exact phrase or term in keyword and  `property:value` search queries.<p> **Note:** Double quotes are automatically added when using `subject:` or `subjecttitle:` and should be skipped. Adding double quotation marks will lead to a pair of PAIR of double quotatioons and will to a KQL query error. |
