...
4. The connector imports the chat message items to the mailbox of a specific user. A new folder named InstantBloomberg will be created in the specific user's mailbox and the items will be imported to it. The connector does by using the value of the *CorporateEmailAddress* property. Every chat message contains this property, which is populated with the email address of every participant of the chat message. In addition to automatic user mapping using the value of the *CorporateEmailAddress* property, you can also define your custom mapping by uploading a mapping file in CSV format. The mapping file should have Bloomberg UUID and corresponing Microsoft 365 mailbox address. If you enable automatic user mapping and provide a custom user mapping, for every chat item the connector will first look into the user mapping file provided by you and if it doesn't find a valid Microsoft 365 user corresponding to the user's Bloomberg UUID, it will look into the *CorporateEmailAddress* property of the chat item. If the connector doesn't find valid Microsoft 365 user from both the user mapping file and the *CorporateEmailAddress* property of the chat item, it will not be imported.

## Before you begin

Many of the implementation steps required to archive Instant Bloomberg data are external to Microsoft 365 and must be completed before you can create the connector in the Security & Compliance Center.
...
- Set up a Bloomberg SFTP (Secure file transfer protocol) site. After working with Bloomberg to set up the SFTP site, data from Instant Bloomberg is uploaded to the SFTP site every day. The connector you create in Step 2 connects to this SFTP site and transfers the chat data to Microsoft 365 mailboxes. SFTP also encrypts the Instant Bloomberg chat data that is sent to Microsoft 365 mailboxes during the transfer process.
...
5. On the **User mapping** page, enable automatic user mapping and provide custom user mapping as required
   ...
   > [!NOTE]
   > The connector imports the chat message items to the mailbox of a specific user. A new folder named InstantBloomberg will be created in the specific user's mailbox and the items will be imported to it. The connector does by using the value of the *CorporateEmailAddress* property. Every chat message contains this property, which is populated with the email address of every participant of the chat message. In addition to automatic user mapping using the value of the *CorporateEmailAddress* property, you can also define your custom mapping by uploading a mapping file in CSV format. The mapping file should have Bloomberg UUID and corresponing Microsoft 365 mailbox address. If you enable automatic user mapping and provide a custom user mapping, for every chat item the connector will first look into the user mapping file provided by you and if it doesn't find a valid Microsoft 365 user corresponding to the user's Bloomberg UUID, it will look into the *CorporateEmailAddress* property of the chat item. If the connector doesn't find valid Microsoft 365 user from both the user mapping file and the *CorporateEmailAddress* property of the chat item, it will not be imported.
