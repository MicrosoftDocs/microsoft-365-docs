...
6. For the page **Choose locations where you want to apply the label**: Select and specify locations for Exchange, SharePoint, and OneDrive. If you don't want to keep the default of **All** included for your chosen locations, select the link to choose specific instances to include, or select the link to choose specific instances to exclude. Then select **Next**.
    ...
    - For the **Exchange** location, the policy is applied according the sender address of the recipients specified. Most of the time, you'll want to keep the default of **All** included and use the advanced rules in the next step to configure conditions to include or exclude recipients in your organization. Otherwise:
        ...
        -  If you keep the default of **All** included but specify users or groups to exclude, email sent by these users will be be exempt from the policy, but not email sent to these users.
