...
- **Example 2: Protecting sensitive information in documents**. Suppose that someone attempts to share a document with guests in a Microsoft Teams channel or chat, and the document contains sensitive information. If you have a DLP policy defined to prevent this, the document won't open for those users. Your DLP policy must include SharePoint and OneDrive in order for protection to be in place. This is an example of DLP for SharePoint that shows up in Microsoft Teams, and therefore requires that users are licensed for Office 365 DLP (included in Office 365 E3), but does not require users to be licensed for Office 365 Advanced Compliance. 
    ...
    In case you don’t want to have a separate DLP policy for SharePoint and OneDrive or want the files shared in Teams to be evaluated under same conditions, you can extend the Teams DLP policy to cover these locations (SharePoint and OneDrive) by selecting ‘Automatic file protection’ from the banner in DLP -> Policies. This will enable all the files shared in Teams chats and channels to be protected with the same rules* that apply to Teams messages. Once enabled, the extension will apply to all existing as well as future Teams DLP policies, thereby not making it mandatory to create separate policies for SharePoint and OneDrive for file protection. 
*Only those Teams policies having rules with conditions or exceptions of ‘content contains’ and ‘content is shared from Microsoft 365’ can be extended for ‘automatic file protection’. If the condition/exception configuration has ‘Sender is’, ‘sender domain is’, ‘recipient is’ and ‘recipient domain is’ present, then the action of extension will fail as these conditions don’t apply to SharePoint and OneDrive
