...
- Data leaks
- Security violation on laptop/personal computer
- Offensive language in communication

ee [Insider risk management policies](insider-risk-management-policies.md) for more details.

### Alerts

Alerts are automatically generated by signals that match policy conditions and are displayed in the alerts dashboard. This dashboard enables a quick view of all alerts needing review, open alerts over time, and alert statistics for your organization. To protect employee privacy and to help reviewers remain objective, employee personal identifiable information (PII) associated with alerts can be automatically pseudonymized if required. All policies with matching alerts are displayed with associated information:
...
See [Insider risk management alerts](insider-risk-management-alerts.md) for more details.
...
Selecting a case on the case dashboard opens the case for investigation and review. This is the heart of the insider risk management workflow and where activity signals, policy conditions, alerts details, and employee details are synthesized into a integrated view for reviewers. The primary investigation tools in this area are:
...
- **Content explorer**: All data files and email messages associated with alert risk activities are automatically captured and displayed in the content explorer. Reviewers can filter and view files amd messages by data source, file type, tags, conversation, and many more attributes.
- **Social graph**: Employees with risky activities may have associates with current or past cases that have connections helpful for reviewers. The social graph automatically displays employees with cases containing alerts that are associated with the employee, sorted by risk severity level.
- **Case notes**: Reviewers provide notes for a case in the Case Notes section. This consolidates all notes in a central view and include reviewer and date submitted information. 

See [Insider risk management cases](insider-risk-management-cases.md) for more details.

SCREENSHOT OF CASE USER ACTIVITY

### Action

After cases are investigated, reviewers can quickly take action to resolve the case or collaborate with other risk stakeholders in your organization. For the most common scenario of employees accidentally or inadvertently violating policy conditions, a reminder notice can be sent to the employee from custom notice templates you can configure for your organization. These notices can serve as simple reminders or can direct the employee to refresher training or guidance to help prevent future risky behavior.

See [Insider risk management notices](insider-risk-management-notices.md) for more details.
