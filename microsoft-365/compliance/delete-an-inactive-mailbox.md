...
- **An inactive mailbox is a type of soft-deleted mailbox.** In Exchange Online, a soft-deleted mailbox is a mailbox that's been deleted but can be recovered within a specific retention period. For normal mailbox that has been deleted will remain in soft-deleted state for 30 days. An inactive mailbox (mailbox with retention hold assigned before it was deleted) will remain in soft-deleted state until the hold expires. Once the hold expires, either via hold duration specified or via manual removal of the hold, the mailbox will no longer be in an inactive state. Instead it will become soft-deleted and remain in Exchange Online for 183 days. This means that the mailbox can be recovered within 183 days of being soft-deleted. After 183 days, a soft-deleted mailbox is marked for permanent deletion and can't be recovered.

- **What happens after you remove the hold on an inactive mailbox?** The mailbox is treated like other soft-deleted mailboxes and is marked for permanent deletion after the 183-day soft-deleted mailbox retention period expires. This retention period starts on the date when the hold was removed from the inactive mailbox. InactiveMailboxRetireTime is set when the mailbox is transitions from being inactive (soft deleted with holds) to no longer being inactive (soft deleted without holds). At that point InactiveMailboxRetireTime is set to the current date when the transition occurred. There is an assistant that runs (MailboxLifeCycle assistant) and it will look for mailboxes that have InactiveMailboxRetireTime set. If InactiveMailboxRetireTime + 183 days is less that the current date then it will purge the mailbox.
