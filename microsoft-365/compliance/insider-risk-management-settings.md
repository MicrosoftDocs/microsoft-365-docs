...
By excluding sensitive info types, you can specify which types map to indicators and triggers involving file-related activities for Endpoint, SharePoint, Teams, OneDrive, and Exchange. For those files that contain any sensitive info types identified here, they will be risk scored but not shown as activities involving content related to sensitive info types. You may want to consider excluding non-sensitive info types for certain indicators that focus on identifying specific sensitive activity. For a complete list, see [Sensitive information type entity definitions](sensitive-information-type-entity-definitions.md).
