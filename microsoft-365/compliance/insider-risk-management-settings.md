...
### Minimum number of daily events to boost score for potentially risky activity

With this setting, you define how many daily events are required to boost the risk score for potentially risky activity that's considered unusual for a user. For example, let's say you enter 25 for this risk booster. If a user averages 10 file downloads over the past 30 days, but a policy detects they downloaded 20 files on one day, the score for that user won't be boosted even though it's unusual for that user because the number of files they downloaded that day was less than the number you entered for this risk booster.
