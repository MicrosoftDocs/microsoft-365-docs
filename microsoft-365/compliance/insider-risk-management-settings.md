...
If an email message contains an HTML-based email signature (a signature that includes a logo image or profile picture), that signature will be added as an attachment if the user doesn't choose the HTML or rich-text-formatting option when sending the email. If the *Sending email with attachments to recipients outside the organization* indicator is selected, the attachment is then scored like any other email attachment sent outside the organization, even if the only thing in the attachment is the email signature. You can exclude attachments from being scored in this situation, and reduce "noisy" alerts, by turning on the **Email signatures** setting. Turning this feature on doesn't completely eliminate noise from email signature attachments. For example, if two people are communicating, and both have email signature attachments, only the originating email attachment is excluded. Also, if someone changes their email signature, the new signature has to be profiled, which can cause alert noise for a short period of time. Insider risk management checks every week for changes to email signatures. 
