...
5. To hash and upload the sensitive data, run the following command in Command Prompt window:
   ...
   If you have some incorrectly formatted data (e.g. data with an invalid number of columns, commas not surrounded in quotes or invalid characters) in your sensitive information table and you still want to import the remaining data while ignoring invalid rows, you can specify a confugurable error threshold by including the /AllowedBadLinesPercentage parameter in the command. The example above specifies a 5% threshold, meaning that the tool will continue to hash and upload the data even if there's up to 5% of invalid rows. The default value for this setting is 1%. 
