...
Note that when you add sensitive information to an email, there may be latency between when the sensitive information is added and when the policy tip appears. Currently, the policy tips do not appear on emails with the condition to detect encryption when they are encrypted via OME.
