...
> [!NOTE]
> Email notifications can be sent only to individual recipients—not groups or distribution lists. Only new content will trigger an email notification. Editing existing content will trigger policy tips, but not an email notification. If an email is sent from a user external to the organization that has set the rule, they will not receive any notification as these notifications only go to internal users.
