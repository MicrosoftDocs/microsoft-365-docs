...
Each type of job is displayed on the **Jobs report (preview)** tab and on the **Jobs** tab of each case in Microsoft Purview eDiscovery (Premium).

## Jobs report (preview)

The **Jobs report (preview)** displays information about all job activities across all cases within eDiscovery and content search. eDiscovery administrators can view job activity details for all cases and customize columns, filters groups, and the time period for the report as needed. Job details can be downloaded and saved as a .csv file.

The jobs report includes the following report widgets:

- **Jobs in progress**: Displays the percentage of all Premium and Standard jobs that are in progress and the concurrent limits for each. When concurrent limtes are reached, additional Premium and Standard jobs are rejected.
- **Premium jobs in queue**: NOT DISPLAYED IN DF TENANT?
- **Daily limit for standard jobs**: Displays the current number of standard job slots available for the current day. Standard jobs are rejected when the daily limit is reached. The limit resets everyday at 00:00 UTC.

SCREENSHOT

## Job types

The following table describes the different job types:

|**Job type**|**Description**|
|:-----------|:--------------|
| Add external data |  |
| Adding data to a review set | A user adds a collection to a review set. This job consists of two sub jobs: <ul><li>**Export** - A list of items in the collection is generated.</li><li>**Ingestion & Indexing** - The items in the collection that match the search query are copied to an Azure Storage location (in a process called *ingestion*) and then those items in the Azure Storage location are reindexed. This new index is used when querying and analyzing items in the data set. <p> For more information, see [Add search results to a review set](add-data-to-review-set.md).|
| Add items to another review set | SAME AS Adding data to another review set? |
| Add items to review set | NEED DESCRIPTION FROM PM |
...
| Applying holds | NEED DESCRIPTION FROM PM - TENSE OF APPLY DIFFERENT FOR FILTER? |
| CaseHoldSyncJob | NEED DESCRIPTION FROM PM |
| Collect and export items | NEED DESCRIPTION FROM PM |
...
| Generate report | NEED DESCRIPTION FROM PM |
| Import custodians | NEED DESCRIPTION FROM PM |
| MovingBetweenWorkingSet | NEED DESCRIPTION FROM PM |
| NormalCaseDiscoveryJob| NEED DESCRIPTION FROM PM |
| NormalCaseIndexJob | NEED DESCRIPTION FROM PM |
| PolicySyncJob | NEED DESCRIPTION FROM PM |
...
| Reindex data sources | NEED DESCRIPTION FROM PM |
| Relevance load | NEED DESCRIPTION FROM PM |
...
| StartAEDExportTask | NEED DESCRIPTION FROM PM |
| TagJob | NEED DESCRIPTION FROM PM |
