...
|copy to other app    |Detects when a user attempts to copy information from a protected item and then paste it into another app, process or item. For Word/Excel/PowerPoint, copying and pasting information among files within the same app, process, or item will be detected by this activity; for other apps, it will be not detected by this activity.|supported|supported         | auditable and restrictable|
