...
> [!NOTE]
> If you have existing policies or custom SITs defined using number based confidence levels(also know as accuracy), then they will automatically be mapped to the three discrete confidence levels - low confidence, medium confidence & high confidence across the SCC UI.
> - All policies with min accuracy or custom SIT patterns with confidence levels between 76 & 100 will be mapped to high confidence. 
> - All policies with min accuracy or custom SIT patterns with confidence levels between 66 & 75 will be mapped to medium confidence.
> - All policies with min accuracy or custom SIT patterns with confidence levels less than equal to 65 will be mapped to low confidence. 
