...
- **Mark the content** when you use Office apps, by adding watermarks, headers, or footers to email or documents that have the label applied. Watermarks can be applied to documents but not email. Example header and watermark:
    ...
    Although you can configure a custom font name and custom font color by RGB code for these content markings (header, footer, watermark), these settings are supported by Azure Information Protection unified labeling client and scanner only. When you use built-in labeling, these settings are ignored.
