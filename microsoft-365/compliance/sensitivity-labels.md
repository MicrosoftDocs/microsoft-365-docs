...
See the following example of how users see an applied sensitivity label in the Excel window bar, and how they can easily change the label if needed, by using the [sensitivity bar](sensitivity-labels-office-apps.md#sensitivity-bar) that's available with the latest versions of Office. The same labels are also available from the **Sensitivity** button on the **Home** tab from the ribbon.
